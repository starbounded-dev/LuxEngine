...
layout (set = 0, binding = 0) uniform texture2D u_FontAtlases[32];
...
{
	float pxRange = 2.0f;
    vec2 unitRange = vec2(pxRange)/vec2(textureSize(sampler2D(u_FontAtlases[int(TexIndex)], r_DefaultSampler), 0));
...
void main()
{
	...
	vec3 msd = SampleDefault(u_FontAtlases[int(TexIndex)], Input.TexCoord).rgb;
