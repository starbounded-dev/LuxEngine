...
layout(binding = 1) uniform texture2D u_InputDepth;
shared float sharedClosestDeviceZ[LOCAL_SIZE * LOCAL_SIZE];

vec4 Gather4(texture2D tex, vec2 bufferUV, float lod)
{
    vec2 uv[4];
    ...
    vec4 depth;
    depth.x = SampleLinearLOD(tex, uv[0], lod).r;
    depth.y = SampleLinearLOD(tex, uv[1], lod).r;
    depth.z = SampleLinearLOD(tex, uv[2], lod).r;
    depth.w = SampleLinearLOD(tex, uv[3], lod).r;
    return depth;
}
...
void main()
{
    ...
    if(u_IsFirstPass)
    {
        imageStore(o_HZB[0], outputPixelPos, SampleLinearLOD(u_InputDepth, bufferUV, 0).xxxx);
    }
    else
    {
