...
layout(binding = 1) uniform textureCube inputTexture;
...
void main(void)
{
	...
	vec2 inputSize = vec2(textureSize(samplerCube(inputTexture, r_LinearSampler), 0));
	...
	for(uint i = 0; i < NumSamples; i++)
	{
		...
		if(cosLi > 0.0) {
			...
			color  += textureLod(samplerCube(inputTexture, r_LinearSampler), Li, mipLevel).rgb * cosLi;
			weight += cosLi;
		}
	}
	color /= weight;
