...
layout(set = 1, binding = 2) uniform texture2D u_InputColor;
layout(set = 1, binding = 3) uniform texture2D u_Normal;
layout(set = 1, binding = 4) uniform texture2D u_HiZBuffer;
layout(set = 1, binding = 5) uniform texture2D u_MetalnessRoughness;
layout(set = 1, binding = 6) uniform texture2D u_VisibilityBuffer;
...
vec4 ConeSampleWeightedColor(vec2 samplePos, float mipLevel)
{
	...
	vec3 color = SampleLinearLOD(u_InputColor, samplePos, mipLevel).rgb;
	float visibility = SampleLinearLOD(u_VisibilityBuffer, samplePos, mipLevel).r;
...
{
	return texelFetch(sampler2D(u_HiZBuffer, r_LinearSampler), coords, lod).x;
}

ivec2 GetDepthMipResolution(int mipLevel)
{
	return ivec2(GetTextureSize(u_HiZBuffer, mipLevel) * u_SSRInfo.HZBUVFactor);
}
...
void main()
{
	...
	const vec4 albedo = SampleLinear(u_InputColor, uv);
	const vec2 metalnessRoughness = SampleLinear(u_MetalnessRoughness, uv).xy;
	
	//const vec4 albedo = vec4(1,1,1,1);//SampleLinear(u_InputColor, uv);
	//const vec2 metalnessRoughness = vec2(1,0.7);

	vec3 normalVS = SampleLinear(u_Normal, uv).xyz;
	//Y Flip from vulkan->nvrhi
	normalVS.y *= -1;
	...
	if (u_SSRInfo.EnableConeTracing)
		...
		totalColor = texelFetch(sampler2D(u_InputColor, r_LinearSampler), ivec2(ray.xy * GetTextureSize(u_InputColor, 0)), 0);
...
#if HBAO_REFLECTION_OCCLUSION
		ao *= SampleLinear(u_HBAOTex, positionSS.xy).x;
#endif
