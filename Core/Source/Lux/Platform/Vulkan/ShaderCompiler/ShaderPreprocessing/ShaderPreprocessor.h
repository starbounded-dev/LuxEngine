...
namespace std {
	template<>
	struct hash<Lux::IncludeData>
	{
		size_t operator()(const Lux::IncludeData& data) const noexcept
		{
...
namespace Lux {
		...
		while (startOfShaderStage != std::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "pragma")
				{
					...
					{
						LUX_CORE_VERIFY(tokens[++index] == ":", "Stage pragma is invalid");
						...
						if (isGuarded && alreadyIncluded)
							...
							LUX_CORE_WARN("\"{}\" Header does not contain a header guard (#pragma once).", fullPath);
	...
	std::map<nvrhi::ShaderType, std::string> ShaderPreprocessor::PreprocessShader(const std::string& source, std::unordered_set<std::string>& specialMacros)
	{
		...
		if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
		{
			...
			LUX_CORE_VERIFY(tokens.size() >= 3 && tokens[1] == "version", "Invalid #version encountered or #version is NOT encounted first.");
		...
		LUX_CORE_VERIFY(stagePositions.size(), "Could not pre-process shader! There are no known stages defined in file.");
