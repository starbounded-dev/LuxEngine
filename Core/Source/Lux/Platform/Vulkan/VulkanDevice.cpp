...
namespace Lux {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			LUX_CORE_INFO_TAG("Renderer", "Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		LUX_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		LUX_CORE_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				LUX_CORE_INFO_TAG("Renderer", "Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					LUX_CORE_INFO_TAG("Renderer", "  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		LUX_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		LUX_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
		...
		if (canEnableAftermath)
		{
			...
			GpuCrashTracker* gpuCrashTracker = lnew GpuCrashTracker();
		...
		LUX_CORE_ASSERT(result == VK_SUCCESS);
	...
	void VulkanCommandPool::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		LUX_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
