...
namespace Lux {
		...
		: m_DebugName(std::move(debugName))
	{
		...
		LUX_CORE_VERIFY(commandBufferCount > 0);
	...
	void VulkanRenderCommandBuffer::Begin()
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			if (instance->m_OwnedBySwapChain)
			{
				...
				// TODO(Yan): commandBuffer = swapChain.GetDrawCommandBuffer(commandBufferIndex);
			}
			else
			{
	...
	void VulkanRenderCommandBuffer::Submit()
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			LUX_CORE_TRACE_TAG("Renderer", "Submitting Render Command Buffer {}", instance->m_DebugName);
