...
namespace Lux::UI {
	...
	{
		Normal,
		Opaque,   // Force alpha to 1.0 for debug visualization
		Depth     // Display single-channel as grayscale (for depth maps)
	};

	// Get texture handles with mode support
	ImTextureID GetTextureID(Ref<Image2D> image, ImageMode mode = ImageMode::Normal);
	ImTextureID GetTextureIDLayer(Ref<Image2D> image, uint32_t layer, ImageMode mode = ImageMode::Normal);
	ImTextureID GetTextureIDMip(Ref<Image2D> image, uint32_t mip, ImageMode mode = ImageMode::Normal);
	ImTextureID GetTextureID(Ref<Texture2D> texture);

	// Image display functions - base, layer, and mip versions
	void Image(Ref<Image2D> image, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), ImageMode mode = ImageMode::Normal);
	void Image(Ref<Image2D> image, uint32_t layer, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), ImageMode mode = ImageMode::Normal);
	void ImageMip(Ref<Image2D> image, uint32_t mip, const ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), ImageMode mode = ImageMode::Normal);
