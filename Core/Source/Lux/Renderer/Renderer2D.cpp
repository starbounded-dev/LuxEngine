...
namespace Lux {
	...
	void Renderer2D::Init()
	{
...

		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_QuadVertexBufferBases[0][i] = lnew QuadVertex[c_MaxVertices];
			}

			uint32_t* quadIndices = lnew uint32_t[c_MaxIndices];
		...
		// Lines
		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_LineVertexBufferBases[0][i] = lnew LineVertex[c_MaxLineVertices];
				m_LineOnTopVertexBufferBases[0][i] = lnew LineVertex[c_MaxLineVertices];
			}

			uint32_t* lineIndices = lnew uint32_t[c_MaxLineIndices];
		...
		// Text
		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_MemoryStats.TotalAllocated += allocationSize;
				m_TextVertexBufferBases[0][i] = lnew TextVertex[c_MaxVertices];
			}

			uint32_t* textQuadIndices = lnew uint32_t[c_MaxIndices];
	...
	void Renderer2D::EndScene()
	{
		...
		LUX_CORE_TRACE_TAG("Renderer", "Renderer2D::EndScene frame {}", frameIndex);
		uint32_t dataSize = 0;
	...
	void Renderer2D::AddQuadBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = lnew QuadVertex[c_MaxVertices];
		}
	}
	...
	void Renderer2D::AddLineBuffer(const bool onTop)
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize);
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = lnew LineVertex[c_MaxLineVertices];
		}
	}
	...
	void Renderer2D::AddTextBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize);
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = lnew TextVertex[c_MaxVertices];
		}
	}
	...
	void Renderer2D::AddCircleBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize);
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = lnew CircleVertex[c_MaxVertices];
		}
	}
