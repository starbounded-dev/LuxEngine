...
namespace StarEngine {
	...
	/*	auto[simpleMeshResult, complexMeshResult] = CookingFactory::CookMesh(colliderAsset->Handle);
		SE_CORE_ASSERT(m_MeshData.find(collisionMesh) != m_MeshData.end());*/
		const auto& meshDataMap = m_MeshData.at(collisionMesh);
		SE_CORE_ASSERT(meshDataMap.find(colliderAsset->Handle) != meshDataMap.end());
		return meshDataMap.at(colliderAsset->Handle);
	}
	...
	{
		/*SE_CORE_INFO_TAG("Physics", "Rebuilding collider cache");

		if (!FileSystem::DeleteFile(Utils::GetCacheDirectory()))
		{
			SE_CORE_ERROR_TAG("Physics", "Failed to delete collider cache!");
			return;
		}
		...
		for (const auto& [filepath, metadata] : assetRegistry)
		{
			...
			if (simpleMeshResult != CookingResult::Success)
				SE_CORE_ERROR_TAG("Physics", "Failed to cook simple collider for '{0}'", metadata.Handle);

			if (complexMeshResult != CookingResult::Success)
				SE_CORE_ERROR_TAG("Physics", "Failed to cook complex collider for '{0}'", metadata.Handle);
		}
		...
		for (const auto& [handle, asset] : memoryAssets)
		{
			...
			if (simpleMeshResult != CookingResult::Success)
				SE_CORE_ERROR_TAG("Physics", "Failed to cook simple collider for '{0}'", handle);

			if (complexMeshResult != CookingResult::Success)
				SE_CORE_ERROR_TAG("Physics", "Failed to cook complex collider for '{0}'", handle);
		}
	
		SE_CORE_INFO_TAG("Physics", "Finished rebuilding collider cache");*/
	}
	...
	{
		SE_CORE_ASSERT(debugMesh->GetAssetType() == AssetType::StaticMesh, "Wrong overload of AddSimpleDebugMesh() called.  Expected Ref<StaticMesh>!");
		if (AssetManager::GetMemoryAsset(colliderAsset->Handle))
	...
	{
		SE_CORE_ASSERT(debugMesh->GetAssetType() == AssetType::Mesh, "Wrong overload of AddSimpleDebugMesh() called.  Expected Ref<Mesh>!");
		if (AssetManager::GetMemoryAsset(colliderAsset->Handle))
	...
	{
		SE_CORE_ASSERT(debugMesh->GetAssetType() == AssetType::StaticMesh, "Wrong overload of AddComplexDebugMesh() called.  Expected Ref<StaticMesh>!");
		if (AssetManager::GetMemoryAsset(colliderAsset->Handle))
	...
	{
		SE_CORE_ASSERT(debugMesh->GetAssetType() == AssetType::Mesh, "Wrong overload of AddComplexDebugMesh() called.  Expected Ref<Mesh>!");
		if (AssetManager::GetMemoryAsset(colliderAsset->Handle))
