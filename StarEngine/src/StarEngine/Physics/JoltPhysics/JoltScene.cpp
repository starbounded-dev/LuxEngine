...
namespace StarEngine {
	...
	JoltScene::JoltScene(const Ref<Scene>& scene)
		: PhysicsScene(scene)
	{
		SE_CORE_VERIFY(s_CurrentInstance == nullptr, "Shouldn't have multiple instances of a physics scene!");
	...
	void JoltScene::Simulate(float ts)
	{
		...
		{
			SE_PROFILE_SCOPE_DYNAMIC("JoltSystem::Update");
		...
		{
			SE_PROFILE_SCOPE_DYNAMIC("JoltScene::SynchronizeTransform");
			...
			for (auto& [entityID, characterController] : m_CharacterControllers)
			{
				...
				Utils::ReorientAnimation(entity, rotation, transformComponent.GetRotation());
			}
	...
	Ref<PhysicsBody> JoltScene::CreateBody(Entity entity, BodyAddType addType)
	{
		if (!entity.HasAny<CompoundColliderComponent, BoxColliderComponent, SphereColliderComponent, CapsuleColliderComponent, MeshColliderComponent>())
		{
			...
			SE_CONSOLE_LOG_ERROR("Entity does not have a collider component!");
			return nullptr;
		}

		if (auto existingBody = GetBody(entity))
			return existingBody;
		...
		switch (addType)
		{
			...
			{
				//SE_CORE_INFO_TAG("Physics", "Adding body immediately.");
	...
	{
		auto entityBody = GetBody(entity);
		...
		if (!entityBody)
		{
			...
			SE_CONSOLE_LOG_ERROR("Entity does not have a body component!");
			return;
		}
	...
	bool JoltScene::CastRay(const RayCastInfo* rayCastInfo, SceneQueryHit& outHit)
	{
		...
		{
			if (rayCastInfo->ExcludedEntities.find(entityID) != rayCastInfo->ExcludedEntities.end())
				continue;
			...
			SE_CORE_VERIFY(id < JPH::BodyID::cMaxBodyIndex, "If you get this notify Peter and tell him to revise this code.");
	...
	bool JoltScene::CastShape(const ShapeCastInfo* shapeCastInfo, SceneQueryHit& outHit)
	{
		...
		default:
			SE_CORE_VERIFY(false, "Cannot cast mesh shapes!");
		}

		SE_CORE_VERIFY(shape, "Failed to create shape?");
		...
		for (const auto& [entityID, characterController] : m_CharacterControllers)
		{
			if (shapeCastInfo->ExcludedEntities.find(entityID) != shapeCastInfo->ExcludedEntities.end())
				continue;
	...
	int32_t JoltScene::OverlapShape(const ShapeOverlapInfo* shapeOverlapInfo, SceneQueryHit** outHits)
	{
		...
		switch (shapeOverlapInfo->GetCastType())
		{
			...
			default:
				SE_CORE_VERIFY(false, "Cannot overlap mesh shapes!");
		}
	...
	{
		auto body = GetBody(entity).As<JoltBody>();
	...
	{
		SE_CORE_VERIFY(entity.HasComponent<CharacterControllerComponent>());
	...
	{
		SE_CORE_VERIFY(s_CurrentInstance != nullptr);
	...
	void JoltScene::ProcessBulkStorage()
	{
		...
		SE_CORE_INFO_TAG("Physics", "Adding {} bodies in bulk!", m_BulkBufferCount);
		...
		{
			SE_CORE_INFO_TAG("Physics", "Optimizing Broad Phase!");
	...
	JoltRayCastBodyFilter::JoltRayCastBodyFilter(JoltScene* scene, const ExcludedEntityMap& excludedEntities)
	{
		...
		{
			if (Ref<PhysicsBody> entityBody = scene->GetBodyByEntityID(entityID); entityBody)
			{
				Ref<JoltBody> joltBody = entityBody.As<JoltBody>();
				m_ExcludedBodies.insert(joltBody->GetBodyID());
			}
			else if (Ref<CharacterController> entityController = scene->GetCharacterControllerByEntityID(entityID); entityController)
			{
				Ref<JoltCharacterController> joltController = entityController.As<JoltCharacterController>();
				m_ExcludedBodies.insert(joltController->GetBodyID());
			}
		}
	}
