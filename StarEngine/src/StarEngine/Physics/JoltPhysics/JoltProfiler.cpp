...
#define SE_ENABLE_JOLT_PROFILING 0

#if SE_ENABLE_PROFILING && defined(JPH_EXTERNAL_PROFILE) && SE_ENABLE_JOLT_PROFILING
	...
	ExternalProfileMeasurement::ExternalProfileMeasurement(const char* inName, uint32 inColor /* = 0 */)
	{
		...
		if (Optick::IsActive(Optick::Mode::DEFAULT) || Optick::IsActive(Optick::Mode::TRACER))
		{
			...
			std::string threadName = StarEngine::Application::IsMainThread() ? "MainThread" : std::format("JoltThread-{}", threadID);
