...
namespace StarEngine {
	...
	void JoltCharacterController::Create()
	{
		...
		// Note (0x): InnerBodyShape is required in order that character controllers will register collisions with other character controllers
		//            There are other possible ways of doing this (e.g. JPH::CharacterVsCharacterCollision interface).
		...
		auto& bi = JoltScene::GetBodyInterface(false);
		bi.SetUserData(m_Controller->GetInnerBodyID(), m_Entity.GetUUID());
		bi.SetObjectLayer(m_Controller->GetInnerBodyID(), m_CollisionLayer);
