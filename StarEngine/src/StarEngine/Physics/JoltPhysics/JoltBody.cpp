...
namespace StarEngine {
	...
	void JoltBody::MoveKinematic(const glm::vec3& targetPosition, const glm::quat& targetRotation, float deltaSeconds)
	{
		...
		SE_CORE_VERIFY(bodyLock.Succeeded());

		JPH::Body& body = bodyLock.GetBody();

		if (body.GetMotionType() != JPH::EMotionType::Kinematic)
		{
			SE_CONSOLE_LOG_ERROR("Cannot call MoveKinematic() on a non-kinematic body!");
			return;
		}
	...
	void JoltBody::Rotate(const glm::vec3& inRotationTimesDeltaTime)
	{
		...
		if (body.GetMotionType() != JPH::EMotionType::Kinematic)
		{
			SE_CONSOLE_LOG_ERROR("Cannot call Rotate() on a non-kinematic body!");
			return;
		}
	...
	void JoltBody::SetGravityEnabled(bool isEnabled)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set gravity on a non-dynamic body!");
			return;
		}

		JPH::BodyLockWrite lock(JoltScene::GetBodyLockInterface(), m_BodyID);
		SE_CORE_VERIFY(lock.Succeeded());
	...
	void JoltBody::AddForce(const glm::vec3& force, EForceMode forceMode, bool forceWake /*= true*/)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot apply force to a non-dynamic body!");
			return;
		}
	...
	void JoltBody::AddTorque(const glm::vec3& torque, bool forceWake /*= true*/)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot apply torque to a non-dynamic body!");
			return;
		}
	...
	void JoltBody::AddRadialImpulse(const glm::vec3& origin, float radius, float strength, EFalloffMode falloff, bool velocityChange)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot apply radial impulse to a non-dynamic body!");
			return;
		}

		JPH::BodyLockRead readLock(JoltScene::GetBodyLockInterface(), m_BodyID);
		SE_CORE_VERIFY(readLock.Succeeded());
		...
		SE_CORE_VERIFY(writeLock.Succeeded());
	...
	float JoltBody::GetMass() const
	{
		if (IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Static body does not have mass!");
			return 0.0f;
		}
	...
	void JoltBody::SetMass(float mass)
	{
		if (IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set mass on a static body!");
			return;
		}
	...
	void JoltBody::SetLinearDrag(float inLinearDrag)
	{
		if (IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set linear drag on a static body!");
			return;
		}
	...
	void JoltBody::SetAngularDrag(float inAngularDrag)
	{
		if (IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set angular drag on a static body!");
			return;
		}
	...
	void JoltBody::SetLinearVelocity(const glm::vec3& inVelocity)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set linear velocity on a non-dynamic body!  Move this body via the entity's transform component.");
			return;
		}
	...
	void JoltBody::SetAngularVelocity(const glm::vec3& inVelocity)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set angular velocity on a non-dynamic body!  Rotate this body via the entity's transform component.");
			return;
		}
	...
	float JoltBody::GetMaxLinearVelocity() const
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot get max linear velocity on a non-dynamic body!");
			return 0.0f;
		}
	...
	void JoltBody::SetMaxLinearVelocity(float inMaxVelocity)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set max linear velocity on a non-dynamic body!");
			return;
		}
	...
	float JoltBody::GetMaxAngularVelocity() const
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot get max angular velocity on a non-dynamic body!");
			return 0.0f;
		}
	...
	void JoltBody::SetMaxAngularVelocity(float inMaxVelocity)
	{
		if (!IsDynamic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set max angular velocity on a non-dynamic body!");
			return;
		}
	...
	void JoltBody::SetSleepState(bool inSleep)
	{
		if (IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot change sleep state of a static body!");
			return;
		}
	...
	void JoltBody::SetTranslation(const glm::vec3& translation)
	{
		...
		if (!body.IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set translation on a non-static body!");
			return;
		}
	...
	void JoltBody::SetRotation(const glm::quat& rotation)
	{
		...
		if (!body.IsStatic())
		{
			SE_CONSOLE_LOG_ERROR("Cannot set rotation on a non-static body!");
			return;
		}
	...
	void JoltBody::CreateStaticBody(JPH::BodyInterface& bodyInterface)
	{
		...
		{
			SE_CORE_INFO_TAG("Physics", "Failed to create static PhysicsBody, no collision shape provided!");
			m_Shapes.clear();
			return;
		}
		...
		if (body == nullptr)
		{
			SE_CORE_ERROR_TAG("Physics", "Failed to create PhysicsBody! This means we don't have enough space to store more RigidBodies!");
			return;
		}
	...
	void JoltBody::CreateDynamicBody(JPH::BodyInterface& bodyInterface)
	{
		...
		if (firstShape == nullptr)
		{
			SE_CORE_INFO_TAG("Physics", "Failed to create dynamic PhysicsBody, no collision shape provided!");
			m_Shapes.clear();
			return;
		}
		...
		if (!PhysicsLayerManager::IsLayerValid(rigidBodyComponent.LayerID))
		{
			...
			SE_CONSOLE_LOG_WARN("Entity '{}' has a RigidBodyComponent with an invalid layer set! Using the Default layer.", m_Entity.Name());
		}
