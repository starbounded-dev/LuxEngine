...
namespace StarEngine {
	...
	static void JoltTraceCallback(const char* format, ...)
	{
		...
		SE_CORE_TRACE_TAG("Physics", buffer);
	}
	...
	{
		SE_CORE_FATAL_TAG("Physics", "{}:{}: ({}) {}", file, line, expression, message != nullptr ? message : "");
		return true;
	}
...
#define SE_ENABLE_MALLOC_ALLOC 0

	void JoltAPI::Init()
	{
		SE_CORE_VERIFY(!s_JoltData, "Can't initialize Jolt multiple times!");
