...
namespace StarEngine {
	...
	void PhysicsScene::AddRadialImpulse(const glm::vec3& origin, float radius, float strength, EFalloffMode falloff, bool velocityChange)
	{
		...
		{
			auto entityBody = GetBodyByEntityID(hitActors[i].HitEntity);
	...
	{
		SE_PROFILE_SCOPE_DYNAMIC("PhysicsSystem::SubStepStrategy");
	...
	{
		SE_PROFILE_FUNCTION("PhysicsScene::PreSimulate");
		...
		// For each kinematic body, call MoveKinematic() so that the kinematic body will end up at the StarEngine Entity's position and rotation.
	...
	{
		SE_PROFILE_FUNCTION("PhysicsScene::PostSimulate");
