...
namespace ImGui {
	...
	bool TreeNodeWithIcon(StarEngine::Ref<StarEngine::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint/* = IM_COL32_WHITE*/)
	{
		...
		// NOTE(Yan): TreeJumpToParentOnPopMask was removed, not sure if we still need this
		...
		if (display_frame)
		{
			...
			RenderNavCursor(frame_bb, id, nav_highlight_flags);
			...
			if (icon)
			{
				...
				auto itemFlags = lastItem.ItemFlags;
		...
		else
		{
			...
			if (hovered || selected)
			{
				//if (held && hovered) SE_CORE_WARN("held && hovered");
				//if(hovered && !selected && !held && !pressed && !toggled) SE_CORE_WARN("hovered && !selected && !held");
				//else if(!selected) SE_CORE_WARN("ImGuiCol_Header");
