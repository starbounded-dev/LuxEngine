...
namespace StarEngine::UI {
	...
	// Minimal stub: opens a popup, shows a search box, and can be closed.
	// TODO: enumerate scripts from ScriptEngine and set `outScript` when user selects one.
	bool Widgets::ScriptSearchPopup(const char* popupId,
						   ...
						   ImVec2 size)
	{
		// Open the popup if caller asked for it
		if (pOpen && *pOpen)
		...
		{
			// Search input
			if (!placeholder) placeholder = "Search scripts...";
			ImGui::PushItemWidth(size.x > 0 ? size.x : -1.0f);
			static char s_filterBuf[256] = {};
			ImGui::InputTextWithHint("##script_search", placeholder, s_filterBuf, sizeof(s_filterBuf));
			ImGui::PopItemWidth();

			// Placeholder contents for now
			ImGui::TextUnformatted("TODO: populate with ScriptEngine script list");
		...
		return changed; // return true when you actually set outScript
	}
}
