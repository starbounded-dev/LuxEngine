...
#include "StarEngine/Editor/AssetEditorPanelInterface.h"
#include "StarEngine/Editor/EditorOperation.h"
...
#define SE_MESSAGE_BOX_OK_BUTTON BIT(0)
#define SE_MESSAGE_BOX_CANCEL_BUTTON BIT(1)
#define SE_MESSAGE_BOX_USER_FUNC BIT(2)
#define SE_MESSAGE_BOX_AUTO_SIZE BIT(3)
	...
	static void ShowMessageBox(const char* title, const std::function<void()>& renderFunction, uint32_t width = 600, uint32_t height = 0, uint32_t minWidth = 0, uint32_t minHeight = 0, uint32_t maxWidth = -1, uint32_t maxHeight = -1, uint32_t flags = SE_MESSAGE_BOX_AUTO_SIZE)
	{
		auto& messageBoxData = s_MessageBoxes[title];
		messageBoxData.Title = std::format("{0}##MessageBox{1}", title, s_MessageBoxes.size() + 1);
		messageBoxData.UserRenderFunction = renderFunction;
		messageBoxData.Flags = SE_MESSAGE_BOX_USER_FUNC | flags;
	...
	static void RenderMessageBoxes()
	{
		for (auto& [key, messageBoxData] : s_MessageBoxes)
		{
			...
			if (messageBoxData.Flags & SE_MESSAGE_BOX_AUTO_SIZE)
			{
			...
			{
				if (messageBoxData.Flags & SE_MESSAGE_BOX_USER_FUNC)
				{
					SE_CORE_VERIFY(messageBoxData.UserRenderFunction, "No render function provided for message box!");
				...
				else
				{
					...
					if (messageBoxData.Flags & SE_MESSAGE_BOX_OK_BUTTON)
					{
						if (ImGui::Button("Ok"))
							ImGui::CloseCurrentPopup();

						if (messageBoxData.Flags & SE_MESSAGE_BOX_CANCEL_BUTTON)
							ImGui::SameLine();
					}

					if (messageBoxData.Flags & SE_MESSAGE_BOX_CANCEL_BUTTON && ImGui::Button("Cancel"))
					{
						ImGui::CloseCurrentPopup();
					}
				}
