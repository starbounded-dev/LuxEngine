...
namespace StarEngine {
	...
	void MaterialAsset::SetAlbedoMap(AssetHandle handle)
	{
		...
		if (handle)
		{
			...
			if (!texture)
			{
				...
				SE_CORE_ASSERT(textureSRGB, "Could not find texture with handle {}", handle); // if this fires, you've passed the wrong handle.  Probably somewhere you retrieved the handle directly from shader.  You need to go through MaterialAsset::GetAlbedoMap()
	...
	{
		SE_CORE_ASSERT(HasMaterial(index));
