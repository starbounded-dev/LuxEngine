...
#include "StarEngine/Renderer/UI/MSDFData.h"
...
namespace StarEngine {
	...
	void Renderer2D::Init()
	{
...

		{
			...
			SE_CORE_VERIFY(m_QuadPass->Validate());
			m_QuadPass->Bake();
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_QuadVertexBufferBases[0][i] = snew QuadVertex[c_MaxVertices];
			}

			uint32_t* quadIndices = snew uint32_t[c_MaxIndices];
		...
		// Lines
		{
...

			{
				...
				SE_CORE_VERIFY(m_LinePass->Validate());
				m_LinePass->Bake();
			}
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_LineVertexBufferBases[0][i] = snew LineVertex[c_MaxLineVertices];
				m_LineOnTopVertexBufferBases[0][i] = snew LineVertex[c_MaxLineVertices];
			}

			uint32_t* lineIndices = snew uint32_t[c_MaxLineIndices];
		...
		// Text
		{
			...
			SE_CORE_VERIFY(m_TextPass->Validate());
			m_TextPass->Bake();
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_MemoryStats.TotalAllocated += allocationSize;
				m_TextVertexBufferBases[0][i] = snew TextVertex[c_MaxVertices];
			}

			uint32_t* textQuadIndices = snew uint32_t[c_MaxIndices];
		...
		// Circles
		{
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
			{
				...
				m_MemoryStats.TotalAllocated += allocationSize;
				m_CircleVertexBufferBases[0][i] = snew CircleVertex[c_MaxVertices];
			}
		}
	...
	void Renderer2D::BeginScene(const glm::mat4& viewProj, const glm::mat4& view, bool depthTest)
	{
		...
		SE_CORE_TRACE_TAG("Renderer", "Renderer2D::BeginScene frame {}", frameIndex);
	...
	void Renderer2D::EndScene()
	{
		...
		SE_CORE_TRACE_TAG("Renderer", "Renderer2D::EndScene frame {}", frameIndex);
		uint32_t dataSize = 0;
	...
	void Renderer2D::AddQuadBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = snew QuadVertex[c_MaxVertices];
		}
	}
	...
	void Renderer2D::AddLineBuffer(const bool onTop)
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize);
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = snew LineVertex[c_MaxLineVertices];
		}
	}
	...
	void Renderer2D::AddTextBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize);
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = snew TextVertex[c_MaxVertices];
		}
	}
	...
	void Renderer2D::AddCircleBuffer()
	{
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			newVertexBuffer[i] = VertexBuffer::Create(allocationSize);
			m_MemoryStats.TotalAllocated += allocationSize;
			newVertexBufferBase[i] = snew CircleVertex[c_MaxVertices];
		}
	}
