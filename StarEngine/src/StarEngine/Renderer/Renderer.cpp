...
namespace StarEngine {
	...
	void Renderer::Init()
	{
		...
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/StarEnginePBR_Static.glsl");
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/StarEnginePBR_Transparent.glsl");
		Renderer::GetShaderLibrary()->Load("Resources/Shaders/StarEnginePBR_Anim.glsl");
		...
		Renderer::Submit([renderCommandBuffer, renderPass, explicitClear]() mutable
			{
				...
				{
					if (bindingSets[i])
						graphicsState.bindings.push_back(bindingSets[i]);
				}
#endif
...
#if TODO
		SE_CORE_ASSERT(renderPass, "RenderPass cannot be null!");
	...
	void Renderer::DispatchCompute(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<ComputePass> computePass, Ref<Material> material, const glm::uvec3& workGroups, Buffer constants)
	{
		...
		Renderer::Submit([renderCommandBuffer, computePass, material, workGroups, constantsBuffer]() mutable
		{
			...
			computePass->Dispatch(workGroups);
		});
#endif
	}
	...
	void Renderer::RenderStaticMeshWithMaterial(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<StaticMesh> staticMesh, Ref<MeshSource> meshSource, uint32_t submeshIndex, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, uint32_t instanceCount, Ref<Material> material, Buffer additionalUniforms)
	{
		...
		{
			SE_PROFILE_FUNCTION("Renderer::RenderStaticMeshWithMaterial");
			SE_SCOPE_PERF("Renderer::RenderStaticMeshWithMaterial");
