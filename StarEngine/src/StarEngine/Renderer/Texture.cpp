...
namespace StarEngine {
	...
	void Texture2D::CreateFromFile(const TextureSpecification& specification, const std::filesystem::path& filepath)
	{
		...
		if (!m_ImageData)
		{
			// TODO(shea): move this to asset manager
	...
	void Texture2D::Invalidate()
	{
		...
		if (!m_ImageData) // TODO(shea): better management for this, probably from texture spec
			imageSpec.Usage = ImageUsage::Storage;
	...
	void Texture2D::GenerateMips()
	{
		...
		{
			// TODO(Yan): compute shader to downsample image
		}
	...
	void TextureCube::Release()
	{
		...
		{
			SE_CORE_TRACE_TAG("Renderer", "Destroying TextureCube");
	...
	void TextureCube::Invalidate()
	{
		...
		textureDesc.mipLevels = GetMipLevelCount();
		textureDesc.arraySize = 6;

		s_TextureCubeReferences[m_Handle.Get()] = this;
	...
	void TextureCube::CopyFromBuffer(const Buffer& buffer, uint32_t mips)
	{
		SE_CORE_VERIFY(false);
#if DEAL
		// SE_CORE_VERIFY(buffer.Size == m_GPUAllocationSize);
