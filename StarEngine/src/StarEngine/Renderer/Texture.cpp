...
namespace StarEngine {
	...
	namespace Utils {
		...
		static nvrhi::SamplerAddressMode NVRHISamplerWrap(TextureWrap wrap)
		{
			...
			SE_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static bool NVRHISamplerFilter(TextureFilter filter)
		{
			...
			SE_CORE_ASSERT(false, "Unknown filter");
			return false;
		}
	...
	void Texture2D::CreateFromFile(const TextureSpecification& specification, const std::filesystem::path& filepath)
	{
		...
		::StarEngine::Log::PrintMessage(::StarEngine::Log::Type::Core, ::StarEngine::Log::Level::Info, "loading image {}" , filepath);
		...
		if (!m_ImageData)
		{
			...
			SE_CORE_ERROR("Failed to load texture from file: {}", filepath);
		...
		SE_CORE_ASSERT(m_Specification.Format != ImageFormat::None);
	...
	{
		::StarEngine::Log::PrintMessage(::StarEngine::Log::Type::Core, ::StarEngine::Log::Level::Info, "loading image from buffer");
	...
	void Texture2D::SetData(Buffer buffer)
	{
		...
		commandList->beginTrackingTextureState(m_Image->GetHandle(), nvrhi::AllSubresources, nvrhi::ResourceStates::Common);

		commandList->writeTexture(m_Image->GetHandle(), 0, 0, buffer.Data, m_Specification.Width);

		commandList->setPermanentTextureState(m_Image->GetHandle(), nvrhi::ResourceStates::ShaderResource);
		...
		SE_CORE_ASSERT(m_ImageData.Data);
	...
	{/*
		...
		{
			SE_CORE_WARN("Texture2D::GenerateMips - Skipping compute-based mip generation for SRGB/SRGBA format texture '{}'. "
		...
		{
			HZ_CORE_WARN("{}: Generating mips for format {}", instance->m_Specification.DebugName, Utils::ImageFormatToString(instance->m_Specification.Format));
	...
	void TextureCube::Release()
	{
		...
		{
			HZ_CORE_TRACE_TAG("Renderer", "Destroying TextureCube");
	...
	{
		SE_CORE_WARN("Destroying TextureCube (LIVE REFS={})", s_TextureCubeReferences.size());
		Release();
	}
	...
	void TextureCube::Invalidate()
	{
		...
		SE_CORE_WARN("Creating TextureCube (LIVE REFS={})", s_TextureCubeReferences.size());
	...
	{/*
		...
		SE_CORE_WARN("TextureCube {}: Generating mips for format {}", m_Specification.DebugName, (int)m_Specification.Format);
