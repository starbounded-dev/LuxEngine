...
//#include "StarEngine/Animation/Animation.h"
//#include "StarEngine/Animation/Skeleton.h"
...
namespace StarEngine {
	...
	struct BoneInfluence
	{
		...
		void AddBoneData(uint32_t boneInfoIndex, float weight)
		{
			...
			{
				SE_CORE_WARN("Vertex bone weight is out of range. We will clamp it to [0, 1] (BoneID={0}, Weight={1})", boneInfoIndex, weight);
			...
			if (weight > 0.0f)
			{
				...
				SE_CORE_WARN("Vertex has more than four bones affecting it, extra bone influences will be discarded (BoneID={0}, Weight={1})", boneInfoIndex, weight);
			}
		}
