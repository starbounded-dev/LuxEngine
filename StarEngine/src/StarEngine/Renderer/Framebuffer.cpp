...
namespace StarEngine {
		...
		: m_Specification(specification)
	{
		...
		SE_CORE_ASSERT(specification.Attachments.Attachments.size());
	...
	{
		// SE_CORE_TRACE("Framebuffer::RT_Invalidate ({})", m_Specification.DebugName);
		...
		for (const auto& attachmentSpec : m_Specification.Attachments.Attachments)
		{
			if (Utils::IsDepthFormat(attachmentSpec.Format))
			{
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					SE_CORE_ASSERT(Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach non-depth image as depth attachment");
			...
			{
				//SE_CORE_ASSERT(!m_Specification.ExistingImage, "Not supported for color attachments");
				...
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					...
					SE_CORE_ASSERT(!Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach depth image as color attachment");
				...
				else
				{
					...
					else
					{
						...
						spec.Width = (uint32_t)(m_Width * m_Specification.Scale);
						spec.Height = (uint32_t)(m_Height * m_Specification.Scale);
						colorAttachmentImage = image.As<Image2D>();
		...
		if (m_DepthAttachmentImage)
		{
			...
			{
				SE_CORE_ASSERT(m_Specification.ExistingImageLayers.size() == 1, "Depth attachments do not support deinterleaving");
			...
			SE_CORE_ASSERT(attachments.back());
		}
