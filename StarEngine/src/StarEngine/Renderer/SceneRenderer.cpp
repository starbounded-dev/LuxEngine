...
namespace StarEngine {
	...
	{
		SE_SCOPE_TIMER("SceneRenderer::Init");
...

		{
			...
			spec.GPUOnly = false;
			const size_t BoneTransformBufferCount = 1024 * 50; // This provides enough for 1024 animated entities at an average of 50 bones each
			m_SBSBoneTransforms = StorageBufferSet::Create(spec, sizeof(glm::mat4) * BoneTransformBufferCount);
			m_BoneTransformsData = snew glm::mat4[BoneTransformBufferCount];
		}
		...
		instanceLayout.SetInstanced(true);
		...
		// Bloom Compute
		{
...

			{
				...
				SE_CORE_VERIFY(m_BloomComputePass->Validate());
		...
		// Directional Shadow pass
		{
			...
			for (uint32_t i = 0; i < m_Specification.NumShadowCascades; i++)
			{
				...
				SE_CORE_VERIFY(m_DirectionalShadowMapPass[i]->Validate());
				...
				SE_CORE_VERIFY(m_DirectionalShadowMapAnimPass[i]->Validate());
		...
		// Non-directional shadow mapping pass
		{
			...
			SE_CORE_VERIFY(m_SpotShadowPass->Validate());
...

		{
			...
			// specific. StarEngine defines Set 0 as per-draw - so usually materials. Sets 1-3 are scene/renderer owned.
			//
			...
			// 		SE_CORE_VERIFY(shadowMapRenderPass[i]->Validate());
			...
			// PreDepth
			{
				...
				SE_CORE_VERIFY(m_PreDepthPass->Validate());
				m_PreDepthPass->Bake();
				...
				SE_CORE_VERIFY(m_PreDepthAnimPass->Validate());
				...
				SE_CORE_VERIFY(m_PreDepthTransparentPass->Validate());
			...
			// Geometry
			{
				...
				pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("StarEnginePBR_Static");
				...
				pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("StarEnginePBR_Transparent");
				...
				pipelineSpecification.Shader = Renderer::GetShaderLibrary()->Get("StarEnginePBR_Anim");
				pipelineSpecification.DepthOperator = DepthCompareOperator::Equal;
			...
			// Selected Geometry isolation (for outline pass)
			{
				...
				SE_CORE_VERIFY(m_SelectedGeometryPass->Validate());
				...
				SE_CORE_VERIFY(m_SelectedGeometryAnimPass->Validate());
...

			{
				...
				// If resources (eg. storage buffers/images) resize, passes need to be invalidated
				// so we can re-create proper descriptors to the newly created replacement resources
				SE_CORE_VERIFY(m_GeometryPass->Validate());
				m_GeometryPass->Bake();
			}

			{
				...
				SE_CORE_VERIFY(m_GeometryAnimPass->Validate());
...

			{
				...
				SE_CORE_VERIFY(m_LightCullingPass->Validate());
		...
		// Deinterleaving
		{
			...
			for (int rp = 0; rp < 2; rp++)
			{
				...
				SE_CORE_VERIFY(m_DeinterleavingPass[rp]->Validate());
		...
		// Hierarchical Z buffer
		{
			...
			SE_CORE_VERIFY(m_HierarchicalDepthPass->Validate());
		...
		// SSR Composite
		{
			...
			SE_CORE_VERIFY(m_SSRCompositePass->Validate());
		...
		// Pre-convolution Compute
		{
			...
			SE_CORE_VERIFY(m_PreConvolutionComputePass->Validate());
		...
		if (m_Specification.EnableEdgeOutlineEffect)
		{
			...
			SE_CORE_VERIFY(m_EdgeDetectionPass->Validate());
		...
		// Composite
		{
			...
			SE_CORE_VERIFY(m_CompositePass->Validate());
		...
		// DOF
		{
			...
			SE_CORE_VERIFY(m_DOFPass->Validate());
			m_DOFPass->Bake();
		}
		...
		// Wireframe
		{
			...
			SE_CORE_VERIFY(m_GeometryWireframePass->Validate());
			...
			SE_CORE_VERIFY(m_GeometryWireframeOnTopPass->Validate());
			...
			SE_CORE_VERIFY(m_GeometryWireframeAnimPass->Validate());
			...
			SE_CORE_VERIFY(m_GeometryWireframeOnTopAnimPass->Validate());
		...
		// Jump Flood (outline)
		{
			...
			SE_CORE_VERIFY(m_JumpFloodInitPass->Validate());
			...
			for (uint32_t i = 0; i < 2; i++)
			{
				...
				SE_CORE_VERIFY(m_JumpFloodPass[i]->Validate());
			...
			if (m_Specification.JumpFloodPass)
			{
				...
				SE_CORE_VERIFY(m_JumpFloodCompositePass->Validate());
		...
		// Grid
		{
			...
			SE_CORE_VERIFY(m_GridRenderPass->Validate());
		...
		// Skybox
		{
			...
			SE_CORE_VERIFY(m_SkyboxPass->Validate());
			m_SkyboxPass->Bake();
		}

		// TODO(Yan): resizeable/flushable
		const size_t TransformBufferCount = 100 * 1024; // 102400 transforms (~4.9MB)
		...
		for (uint32_t i = 0; i < framesInFlight; i++)
		{
			...
			m_SubmeshTransformBuffers[i].Data = snew TransformVertexData[TransformBufferCount];
		}
		...
		// GTAO
		{
			...
			//SE_CORE_VERIFY(m_GTAOComputePass->Validate());
		...
		{
			{
				...
				SE_CORE_VERIFY(m_GTAODenoisePass[0]->Validate());
				...
				SE_CORE_VERIFY(m_GTAODenoisePass[1]->Validate());
			...
			// GTAO Composite
			{
				...
				SE_CORE_VERIFY(m_AOCompositePass->Validate());
		...
		// SSR
		{
			...
			SE_CORE_VERIFY(m_SSRPass->Validate());
			//m_SSRPass->Bake();
		}
	...
	{
		sdelete[] m_BoneTransformsData;
		for (auto& transformBuffer : m_SubmeshTransformBuffers)
			sdelete[] transformBuffer.Data;
	}
	...
	void SceneRenderer::InitOptions()
	{
		...
		// Special macros are strictly starting with "__SE_"
		Renderer::SetGlobalMacroInShaders("__SE_REFLECTION_OCCLUSION_METHOD", std::format("{}", (int)m_Options.ReflectionOcclusionMethod));
		//Renderer::SetGlobalMacroInShaders("__SE_GTAO_COMPUTE_BENT_NORMALS", std::format("{}", (int)m_Options.GTAOBentNormals));
	}
	...
	{
		SE_CORE_ASSERT(!m_Active, "Can't change scenes while rendering");
		m_Scene = scene;
	}
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::BeginScene");

		SE_CORE_ASSERT(m_Scene);
		SE_CORE_ASSERT(!m_Active);
		m_Active = true;
		...
		if (pointLightsVec.data()) std::memcpy(pointLightData.PointLights, pointLightsVec.data(), lightEnvironment.GetPointLightsSize()); //(Karim) Do we really have to copy that?
		...
		if (spotLightsVec.data()) std::memcpy(spotLightData.SpotLights, spotLightsVec.data(), lightEnvironment.GetSpotLightsSize()); //(Karim) Do we really have to copy that?
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::EndScene");
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SubmitMesh");
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SubmitStaticMesh");
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SubmitSelectedMesh");
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SubmitSelectedStaticMesh");
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SubmitStaticDebugMesh");
		SE_CORE_VERIFY(staticMesh);
		SE_CORE_VERIFY(meshSource);
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::ClearPass");
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::ShadowMapPass");

		uint32_t frameIndex = Renderer::GetCurrentFrameIndex();
		m_GPUTimeQueries.DirShadowMapPassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		for (uint32_t i = 0; i < m_Specification.NumShadowCascades; i++)
		{
			...
			{
				SE_CORE_VERIFY(m_MeshTransformMap.find(mk) != m_MeshTransformMap.end());
			...
			for (auto& [mk, dc] : m_ShadowPassDrawList)
			{
				...
				const auto& transformData = m_MeshTransformMap.at(mk);
				if (!dc.IsRigged)
					Renderer::RenderMeshWithMaterial(m_CommandBuffer, m_ShadowPassPipelines[i], dc.Mesh, dc.MeshSource, dc.SubmeshIndex, m_SubmeshTransformBuffers[frameIndex].Buffer, transformData.TransformOffset, 0, 0, dc.InstanceCount, m_ShadowPassMaterial, cascade);
			}
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.DirShadowMapPassQuery);
	}

	void SceneRenderer::SpotShadowMapPass()
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SpotShadowMapPass");

		uint32_t frameIndex = Renderer::GetCurrentFrameIndex();
		m_GPUTimeQueries.SpotShadowMapPassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		for (uint32_t i = 0; i < 1; i++)
		{
			...
			for (auto& [mk, dc] : m_ShadowPassDrawList)
			{
				...
				{
					Renderer::RenderMeshWithMaterial(m_CommandBuffer, m_SpotShadowPassPipeline, dc.Mesh, dc.MeshSource, dc.SubmeshIndex, m_SubmeshTransformBuffers[frameIndex].Buffer, transformData.TransformOffset, 0, 0, dc.InstanceCount, m_SpotShadowPassMaterial, lightIndex);
				}
			}

		}
		Renderer::EndRenderPass(m_CommandBuffer);
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.SpotShadowMapPassQuery);
	}

	void SceneRenderer::PreDepthPass()
	{
		SE_PROFILE_FUNCTION("SceneRenderer::PreDepthPass");

		uint32_t frameIndex = Renderer::GetCurrentFrameIndex();
		m_GPUTimeQueries.DepthPrePassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.DepthPrePassQuery);
	}

	void SceneRenderer::HZBCompute()
	{
		SE_PROFILE_FUNCTION("SceneRenderer::HZBCompute");

		m_GPUTimeQueries.HierarchicalDepthQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.HierarchicalDepthQuery);
	}

	void SceneRenderer::PreIntegration()
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SkyboxPass");

		m_GPUTimeQueries.PreIntegrationQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.PreIntegrationQuery);
	}

	void SceneRenderer::LightCullingPass()
	{
		m_GPUTimeQueries.LightCullingPassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		SceneRenderer::EndGPUPerfMarker(m_CommandBuffer);
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.LightCullingPassQuery);
	}
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::GeometryPass");

		uint32_t frameIndex = Renderer::GetCurrentFrameIndex();

		m_GPUTimeQueries.GeometryPassQuery = m_CommandBuffer->BeginTimerQuery();
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::PreConvolutionCompute");
		...
		m_GPUTimeQueries.PreConvolutionQuery = m_CommandBuffer->BeginTimerQuery();
	...
	void SceneRenderer::GTAOCompute()
	{
		...
		m_GPUTimeQueries.GTAOPassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.GTAOPassQuery);
	}
	...
	void SceneRenderer::GTAODenoiseCompute()
	{
		...
		m_GPUTimeQueries.GTAODenoisePassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.GTAODenoisePassQuery);
	}
	...
	void SceneRenderer::AOComposite()
	{
		...
		m_GPUTimeQueries.AOCompositePassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		Renderer::EndRenderPass(m_CommandBuffer);
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.AOCompositePassQuery);
	}

	void SceneRenderer::JumpFloodPass()
	{
		SE_PROFILE_FUNCTION("SceneRenderer::JumpFloodPass");

		m_GPUTimeQueries.JumpFloodPassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.JumpFloodPassQuery);
	}

	void SceneRenderer::SSRCompute()
	{
		SE_PROFILE_FUNCTION("SceneRenderer::SSRCompute");

		const Buffer pushConstantsBuffer(&m_SSROptions, sizeof(SSROptionsUB));

		m_GPUTimeQueries.SSRQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.SSRQuery);
	}
	...
	void SceneRenderer::SSRCompositePass()
	{
		...
		m_GPUTimeQueries.SSRCompositeQuery = m_CommandBuffer->BeginTimerQuery();
		...
		Renderer::EndRenderPass(m_CommandBuffer);
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.SSRCompositeQuery);
	}
	...
	void SceneRenderer::BloomCompute()
	{
		...
		m_GPUTimeQueries.BloomComputePassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.BloomComputePassQuery);
	}
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::CompositePass");

		uint32_t frameIndex = Renderer::GetCurrentFrameIndex();

		m_GPUTimeQueries.CompositePassQuery = m_CommandBuffer->BeginTimerQuery();
		...
		m_CommandBuffer->EndTimerQuery(m_GPUTimeQueries.CompositePassQuery);
		...
		if (m_Options.ShowSelectedInWireframe)
		{
			...
			for (auto& [mk, dc] : m_SelectedMeshDrawList)
			{
				...
				{
					const auto& transformData = m_MeshTransformMap.at(mk);
					Renderer::RenderMeshWithMaterial(m_CommandBuffer, m_GeometryWireframePass->GetPipeline(), dc.Mesh, dc.MeshSource, dc.SubmeshIndex, m_SubmeshTransformBuffers[frameIndex].Buffer, transformData.TransformOffset + dc.InstanceOffset * sizeof(TransformVertexData), 0, 0, dc.InstanceCount, m_WireframeMaterial);
				}
			}
	...
	{
		SE_PROFILE_FUNCTION("SceneRenderer::PreRender");
		...
		auto data = m_SubmeshTransformBuffers[frameIndex].Data;
		SE_CORE_WARN("Writing {} bytes", offset * sizeof(TransformVertexData));
		/*Ref<SceneRenderer> instance = this;
		Renderer::Submit([instance, offset]() mutable
		{
			uint32_t frameIndex = Renderer::RT_GetCurrentFrameIndex();
			instance->m_SubmeshTransformBuffers[frameIndex].Buffer->RT_SetData(instance->m_SubmeshTransformBuffers[frameIndex].Data, offset * sizeof(TransformVertexData));
		});*/
		...
		for (auto& [key, boneTransformsData] : m_MeshBoneTransformsMap)
		{
			...
			memcpy(&m_BoneTransformsData[index], boneTransformsData.BoneTransformsData.data(), boneTransformsData.BoneTransformsData.size() * sizeof(glm::mat4));
		...
		if (index > 0)
		{
			...
			{
				instance->m_SBSBoneTransforms->RT_Get()->RT_SetData(instance->m_BoneTransformsData, static_cast<uint32_t>(index * sizeof(glm::mat4)));
			});
		}
	}
	...
	void SceneRenderer::CopyToBoneTransformStorage(const MeshKey& meshKey, const Ref<MeshSource>& meshSource, const std::vector<glm::mat4>& boneTransforms)
	{
		...
		{
			std::fill_n(std::back_inserter(boneTransformsMap.BoneTransformsData), stride, glm::identity<glm::mat4>());
		}
		else
		{
