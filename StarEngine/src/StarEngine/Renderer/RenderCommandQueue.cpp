...
#define SE_RENDER_TRACE(...) SE_CORE_TRACE(__VA_ARGS__)

namespace StarEngine {

	RenderCommandQueue::RenderCommandQueue()
	{
		m_CommandBuffer = snew uint8_t[10 * 1024 * 1024]; // 10mb buffer
	...
	void* RenderCommandQueue::Allocate(RenderCommandFn fn, uint32_t size)
	{
		...
		// SE_CORE_VERIFY(!RenderThread::IsCurrentThreadRT());
	...
	{
		//SE_RENDER_TRACE("RenderCommandQueue::Execute -- {0} commands, {1} bytes", m_CommandCount, (m_CommandBufferPtr - m_CommandBuffer));
