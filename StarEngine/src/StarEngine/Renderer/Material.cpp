...
namespace StarEngine {
	...
	void Material::Init()
	{
		...
		SE_CORE_VERIFY(m_DescriptorSetManager.Validate());
	...
	void Material::Set(const std::string& name, const Ref<Image2D>& image)
	{
		m_DescriptorSetManager.SetInput(name, image);
	}

	void Material::Set(const std::string& name, const Ref<ImageView>& image)
	{
		m_DescriptorSetManager.SetInput(name, image);
	}
