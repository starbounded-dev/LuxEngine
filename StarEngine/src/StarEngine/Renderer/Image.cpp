...
namespace StarEngine {
	...
	void Image2D::RT_Invalidate()
	{
		...
		textureDesc.mipLevels = 1;// m_Specification.Mips;
	...
	{
		SE_CORE_VERIFY(m_Specification.Transfer, "Image must be created with ImageSpecification::Transfer enabled!");
	...
	void Image2D::CopyToHostBuffer(Buffer& buffer) const
	{
		...
		for (uint32_t mip = 0; mip < mipCount; mip++)
		{
			...
			SE_CORE_VERIFY(bufferSize == (rowPitch * m_Specification.Height));
		...
		SE_CORE_WARN("MemReq = {} ({})", memReqs.size, memReqs.alignment);
		SE_CORE_WARN("Expected size = {}", bufferSize);
#endif
	...
	void ImageView::RT_Invalidate()
	{
		...
		m_TextureSubresourceSet.numMipLevels = 1;
		m_TextureSubresourceSet.baseArraySlice = 0;
		m_TextureSubresourceSet.numArraySlices = m_Specification.Image->GetSpecification().Layers;
	}
