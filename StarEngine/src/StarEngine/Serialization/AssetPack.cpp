...
#include "StarEngine/Audio/SoundObject.h"
...
#define DEBUG_PRINT 1
		...
		if (sceneCount == 0)
		{
			SE_CONSOLE_LOG_ERROR("There are no scenes in the project.  Nothing to serialize to asset pack!");
			return nullptr;
		}
		...
		for (const auto sceneHandle : sceneHandles)
		{
			...
			SE_CORE_TRACE("Deserializing Scene: {}", metadata.FilePath);
			if (serializer.Deserialize(Project::GetActiveAssetDirectory() / metadata.FilePath))
			{
				...
				SE_CORE_TRACE("  Scene has {} used assets", sceneAssetList.size());
			...
			{
				SE_CONSOLE_LOG_ERROR("Failed to deserialize Scene: {} ({})", metadata.FilePath, sceneHandle);
			}
		...
		SE_CONSOLE_LOG_INFO("Project contains {} used assets", fullAssetList.size());

#if DEBUG_PRINT
		SE_CORE_TRACE("Complete AssetPack:");
		...
		for (AssetHandle handle : fullAssetList)
		{
			...
			SE_CORE_TRACE("{}: {} ({}{})", Utils::AssetTypeToString(AssetManager::GetAssetType(handle)), handle,
		...
		SE_CORE_TRACE_TAG("Asset Pack", "Serialized Assets:");
		for (const auto& [handle, info] : serializedAssets)
		{
			const auto& metadata = Project::GetEditorAssetManager()->GetMetadata(handle);
			SAE_CORE_TRACE_TAG("Asset Pack", "{}: {} (offset = {}, size = {})", Utils::AssetTypeToString(metadata.Type), metadata.FilePath, info.PackedOffset, info.PackedSize);
		}
...
#ifndef SE_DIST
		{
			SE_CORE_INFO_TAG("Asset Pack", "-----------------------------------------------------");
			SE_CORE_INFO_TAG("Asset Pack", "AssetPack Dump {}", assetPack->m_Path);
			SE_CORE_INFO_TAG("Asset Pack", "-----------------------------------------------------");
			std::unordered_map<AssetType, uint32_t> typeCounts;
			std::unordered_set<AssetHandle> duplicatePreventionSet;
			for (const auto& [sceneHandle, sceneInfo] : index.Scenes)
			{
				SE_CORE_INFO_TAG("Asset Pack", "Scene {}:", sceneHandle);
				for (const auto& [assetHandle, assetInfo] : sceneInfo.Assets)
				{
					SE_CORE_INFO_TAG("Asset Pack", "  {} - {}", Utils::AssetTypeToString((AssetType)assetInfo.Type), assetHandle);
			...
			SE_CORE_INFO_TAG("Asset Pack", "-----------------------------------------------------");
			SE_CORE_INFO_TAG("Asset Pack", "Summary:");
			for (const auto& [type, count] : typeCounts)
			{
				SE_CORE_INFO_TAG("Asset Pack", "  {} {}", count, Utils::AssetTypeToString(type));
			}
			SE_CORE_INFO_TAG("Asset Pack", "-----------------------------------------------------");
		}
#endif
