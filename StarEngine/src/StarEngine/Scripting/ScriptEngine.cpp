...
namespace StarEngine {
	...
	{
		SE_CONSOLE_LOG_ERROR("C# Exception: {}", message);
	}
	...
	void OnCoralMessage(std::string_view message, Coral::MessageLevel level)
	{
		switch (level)
		{
			case Coral::MessageLevel::Trace:
				SE_CORE_TRACE_TAG("Scripting", "{}", std::string(message));
				break;
			case Coral::MessageLevel::Info:
				SE_CORE_INFO_TAG("Scripting", "{}", std::string(message));
				break;
			case Coral::MessageLevel::Warning:
				SE_CORE_WARN_TAG("Scripting", "{}", std::string(message));
				break;
			case Coral::MessageLevel::Error:
				SE_CORE_ERROR_TAG("Scripting", "{}", std::string(message));
				break;
		}
	}
		...
		switch (initStatus)
		{
			case Coral::CoralInitStatus::CoralManagedNotFound:
			{
				...
				int response = MessageBoxA(nullptr,
					message.c_str(),
					"StarEngine C# Scripting Engine Initialization Failure", MB_OK | MB_ICONERROR
				);
#else
				SE_CORE_ERROR("StarEngine C# Scripting Engine Initialization Failure: {}", message);
#endif
				break;
			}
...
#else
				SE_CORE_ERROR("StarEngine C# Scripting Engine Initialization Failure: Failed to initialize Coral.Managed");
#endif
				break;
			}
			case Coral::CoralInitStatus::DotNetNotFound:
			{
#if defined(SE_PLATFORM_WINDOWS)
				int response = MessageBoxA(nullptr,
					"StarEngine requires .NET 9 or higher!\n\n"
					...
					"StarEngine C# Scripting Engine Initialization Failure",
				...
				system("xdg-open https://dotnet.microsoft.com/en-us/download/dotnet");
#endif
				break;
			}
			default:
	...
	void ScriptEngine::Initialize(Ref<Project> project)
	{
		...
		auto scriptCorePath = (std::filesystem::current_path() / "Resources" / "Scripts" / "StarEngine-ScriptCore.dll").string();
				...
				for (auto& fieldInfo : type->GetFields())
				{
					...
					if (fieldInfo.GetAccessibility() != Coral::TypeAccessibility::Public)
					{
						...
						auto found = std::ranges::find_if(attributes, [](Coral::Attribute& attribute)
						{
							...
							return name == "StarEngine.ShowInEditorAttribute";
						});
