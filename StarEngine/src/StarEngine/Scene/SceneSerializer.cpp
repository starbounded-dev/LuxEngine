...
namespace StarEngine {
	...
	void SceneSerializer::SerializeEntity(YAML::Emitter& out, Entity entity, Ref<Scene> scene)
	{
		...
		if (entity.HasComponent<TileRendererComponent>())
		{
			...
			out << YAML::Key << "StaticMesh" << YAML::Value << trc.StaticMesh;

			out << YAML::Key << "Width" << YAML::Value << trc.Width;
			out << YAML::Key << "Height" << YAML::Value << trc.Height;

			auto materialTable = trc.Materials;
			out << YAML::Key << "MaterialCount" << YAML::Value << (uint32_t)trc.Materials.size();
			out << YAML::Key << "Materials" << YAML::BeginSeq;
			for (size_t i = 0; i < trc.Materials.size(); i++)
			{
				const auto& materialTable = trc.Materials[i];
				out << YAML::BeginMap;
				out << YAML::Key << "MaterialIndex" << YAML::Value << i;
				if (materialTable->GetMaterialCount() > 0)
				{
					out << YAML::Key << "MaterialTable" << YAML::Value << YAML::BeginMap; // MaterialTable
					for (uint32_t i = 0; i < materialTable->GetMaterialCount(); i++)
	...
	void SceneSerializer::DeserializeEntities(YAML::Node& entitiesNode, Ref<Scene> scene)
	{
		for (auto entity : entitiesNode)
		{
			...
			if (scriptComponent)
			{
				...
				{
					HZ_CORE_ERROR_TAG("ScriptEngine", "Failed to deserialize ScriptComponent on entity '{}' : {}", uuid, e.what());
				}
			}
			...
			auto tileRendererComponent = entity["TileRendererComponent"];
			if (tileRendererComponent)
			{
				auto& component = deserializedEntity.AddComponent<TileRendererComponent>();
				if (tileRendererComponent["StaticMesh"])
				{
					AssetHandle assetHandle = tileRendererComponent["StaticMesh"].as<AssetHandle>();
					if (AssetManager::IsAssetHandleValid(assetHandle))
						component.StaticMesh = assetHandle;
				}
				component.Width = tileRendererComponent["Width"].as<uint32_t>();
				component.Height = tileRendererComponent["Height"].as<uint32_t>();
				
				uint32_t materialCount = tileRendererComponent["MaterialCount"].as<uint32_t>();
				component.Materials.resize(materialCount);
				auto materialsNode = tileRendererComponent["Materials"];
				for (auto materialTable : materialsNode)
				{
					uint32_t materialIndex = materialTable["MaterialIndex"].as<uint32_t>();
					component.Materials[materialIndex] = Ref<MaterialTable>::Create();
					if (materialTable["MaterialTable"])
					{
						YAML::Node materialTableNode = materialTable["MaterialTable"];
						for (auto materialEntry : materialTableNode)
						{
							uint32_t index = materialEntry.first.as<uint32_t>();
							AssetHandle materialAsset = materialEntry.second.as<AssetHandle>(0);
							if (materialAsset && AssetManager::IsAssetHandleValid(materialAsset))
								component.Materials[materialIndex]->SetMaterial(index, materialAsset);
						}
					}
