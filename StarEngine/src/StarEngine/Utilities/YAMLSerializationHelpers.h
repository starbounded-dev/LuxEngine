...
namespace YAML {
	...
	template<>
	struct convert<StarEngine::AssetHandle>
	{
		static Node encode(const StarEngine::AssetHandle& rhs)
		{
			Node node;
		...
		static bool decode(const Node& node, StarEngine::AssetHandle& rhs)
		{
			if(!node.IsScalar())
				return false;
	...
	private:/*
		...
		static Node encodeObject(const choc::value::ValueView& object)
		{
			...
			const bool isUUID = StarEngine::Utils::IsAssetHandle(object);
			if (isUUID)
			{
				const auto& value = object["Value"];
				SE_CORE_ASSERT(value.isInt64());
	...
	private:/*
		...
		static bool decodeObject(const Node& node, choc::value::Value& rhs)
		{
			...
			// So that in the readable YAML it is shown as the correct uint64_t,
			// while for choc::Value it's casted to int64_t
			const bool isUUID = StarEngine::Utils::IsAssetHandle(rhs) && node["Value"];
			if (isUUID)
			{
...
namespace StarEngine {
	...
	inline YAML::Emitter& operator<<(YAML::Emitter& out, const glm::bvec2& v)
	{
		out << YAML::Flow;
		out << YAML::BeginSeq << v.x << v.y << YAML::EndSeq;
		return out;
	...
	inline YAML::Emitter& operator<<(YAML::Emitter& out, const glm::bvec3& v)
	{
		out << YAML::Flow;
		out << YAML::BeginSeq << v.x << v.y << v.z << YAML::EndSeq;
		return out;
	...
	inline YAML::Emitter& operator<<(YAML::Emitter& out, const glm::bvec4& v)
	{
		out << YAML::Flow;
		out << YAML::BeginSeq << v.x << v.y << v.z << v.w << YAML::EndSeq;
		return out;
