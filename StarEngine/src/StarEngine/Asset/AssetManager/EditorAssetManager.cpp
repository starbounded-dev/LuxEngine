...
#include "StarEngine/Asset/AssetExtensions.h"
...
namespace StarEngine {
	...
	{
		SE_PROFILE_FUNCTION("EditorAssetManager::GetAsset");
		SE_SCOPE_PERF("AssetManager::GetAsset");
	...
	{
#if ASYNC_ASSETS
		SE_PROFILE_FUNCTION("EditorAssetManager::GetAssetAsync");
		SE_SCOPE_PERF("AssetManager::GetAssetAsync");
		...
		{
			SE_CORE_VERIFY(m_LoadedAssets.contains(assetHandle));
	...
	bool EditorAssetManager::ReloadData(AssetHandle assetHandle)
	{
		...
		if (!metadata.IsValid())
		{
			SE_CORE_ERROR("Trying to reload invalid asset");
			return false;
		}
		...
		{
			SE_CORE_INFO_TAG("AssetManager", "RELOADING ASSET - {}", metadata.FilePath.string());
			...
			if (metadata.IsDataLoaded)
			{
				...
				SE_CORE_INFO_TAG("AssetManager", "Finished reloading asset {}", metadata.FilePath.string());
				UpdateDependents(assetHandle);
				Application::Get().DispatchEvent<AssetReloadedEvent, /*DispatchImmediately=*/true>(assetHandle);
			}
			else
			{
				SE_CORE_ERROR_TAG("AssetManager", "Failed to reload asset {}", metadata.FilePath.string());
			}
		}
	...
	{
		SE_PROFILE_FUNCTION("EditorAssetManager::EnsureAllLoadedCurrent");
	...
	{
		SE_PROFILE_FUNCTION("EditorAssetManager::IsAssetValid");
		SE_SCOPE_PERF("AssetManager::IsAssetValid");
	...
	std::unordered_set<StarEngine::AssetHandle> EditorAssetManager::GetDependencies(AssetHandle handle)
	{
		bool registered = false;
		std::unordered_set<StarEngine::AssetHandle> result;
		{
		...
		SE_CORE_ASSERT(registered || (GetMetadata(handle).Handle == 0), "asset dependencies are not registered!");
	...
	void EditorAssetManager::SyncWithAssetThread()
	{
		...
		{
			SE_CORE_ASSERT(alr.Asset->Handle == alr.Metadata.Handle, "AssetHandle mismatch in AssetLoadResponse");
		...
		if (metadata.IsValid())
		{
			...
			else
			{
				if (Application::IsMainThread())
				{
					...
					SE_CORE_INFO_TAG("AssetManager", "LOADING ASSET - {}", metadata.FilePath.string());
					if (AssetImporter::TryLoadData(metadata, asset))
					{
						...
						SE_CORE_INFO_TAG("AssetManager", "Finished loading asset {}", metadata.FilePath.string());
					}
					else
					{
						SE_CORE_ERROR_TAG("AssetManager", "Failed to load asset {}", metadata.FilePath.string());
					}
				}
				else
				{
	...
	{
		SE_CORE_INFO("[AssetManager] Loading Asset Registry");
		...
		if (!handles)
		{
			SE_CORE_ERROR("[AssetManager] Asset Registry appears to be corrupted!");
		...
		for (auto entry : handles)
		{
			...
			{
				SE_CORE_WARN_TAG("AssetManager", "Mismatch between stored AssetType and extension type when reading asset registry!");
				metadata.Type = GetAssetTypeFromPath(filepath);
			}

			if (!FileSystem::Exists(GetFileSystemPath(metadata)))
			{
				SE_CORE_WARN("[AssetManager] Missing asset '{0}' detected in registry file, trying to locate...", metadata.FilePath);
				...
				for (auto& pathEntry : std::filesystem::recursive_directory_iterator(Project::GetActiveAssetDirectory()))
				{
					...
					if (bestScore > 0)
						SE_CORE_WARN("[AssetManager] Multiple candidates found...");
					...
					SE_CORE_WARN("'{0}' has a score of {1}, best score is {2}", path.string(), score, bestScore);
				...
				{
					SE_CORE_ERROR("[AssetManager] Failed to locate a potential match for '{0}'", metadata.FilePath);
					continue;
				}
				...
				SE_CORE_WARN("[AssetManager] Found most likely match '{0}'", metadata.FilePath);
			}

			if (metadata.Handle == 0)
			{
				SE_CORE_WARN("[AssetManager] AssetHandle for {0} is 0, this shouldn't happen.", metadata.FilePath);
				continue;
			}
		...
		SE_CORE_INFO("[AssetManager] Loaded {0} asset entries", m_AssetRegistry.Count());
	}
	...
	void EditorAssetManager::WriteRegistryToFile()
	{
		...
		SE_CORE_INFO("[AssetManager] serializing asset registry with {0} entries", sortedMap.size());
