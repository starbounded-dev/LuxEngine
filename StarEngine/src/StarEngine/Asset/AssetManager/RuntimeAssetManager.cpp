...
namespace StarEngine {
	...
	{
		SE_PROFILE_FUNCTION("RuntimeAssetManager::GetAsset");
	...
	{
#if ASYNC_ASSETS
		SE_PROFILE_FUNCTION("RuntimeAssetManager::GetAssetAsync");
		SE_SCOPE_PERF("AssetManager::GetAsset");
	...
	{
		SE_PROFILE_FUNCTION("RuntimeAssetManager::IsAssetValid");
	...
	std::unordered_set<StarEngine::AssetHandle> RuntimeAssetManager::GetAllAssetsWithType(AssetType type)
	{
	...
	void RuntimeAssetManager::SyncWithAssetThread()
	{
		...
		for (const auto& asset : freshAssets)
		{
			...
			SE_CORE_VERIFY(m_PendingAssets.contains(asset->Handle));
