...
//#include "StarEngine/Script/ScriptAsset.h"
...
#include "StarEngine/Serialization/Serialization.h"
...
namespace StarEngine {
	...
	std::string GetYAML(const AssetMetadata& metadata)
	{
	...
	{
		FileStreamReader reader(Project::GetEditorAssetManager()->GetFileSystemPath(metadata));
		std::string data;

		if (!reader || !reader.ReadAsString(data))
			return false;
		...
		if (!soundConfig) [[unlikely]]
			return false;

		if (DeserializeFromYAML(data, soundConfig))
		{
			asset = soundConfig;
			asset->Handle = metadata.Handle;
	...
	std::string SoundConfigSerializer::SerializeToYAML(Ref<SoundConfig> soundConfig) const
	{
		...
		HZ_SERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->AirAbsorptionEnabled, out);
		HZ_SERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->SpreadFromSourceSize, out);
	...
	bool SoundConfigSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SoundConfig> targetSoundConfig) const
	{
		...
		if (spConfigData)
		{
			...
			HZ_DESERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->AirAbsorptionEnabled, spConfigData, true);
			HZ_DESERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->SpreadFromSourceSize, spConfigData, true);
	...
	//////////////////////////////////////////////////////////////////////////////////
	// SpatializationConfigSerializer
	//////////////////////////////////////////////////////////////////////////////////

	void SpatializationConfigSerializer::Serialize(const AssetMetadata& metadata, const Ref<Asset>& asset) const
	{
		if (Ref<SpatializationConfig> spatializationConfig = asset.As<SpatializationConfig>())
		{
			std::string yamlString = SerializeToYAML(spatializationConfig);
	...
	bool SpatializationConfigSerializer::TryLoadData(const AssetMetadata& metadata, Ref<Asset>& asset) const
	{
		FileStreamReader reader(Project::GetEditorAssetManager()->GetFileSystemPath(metadata));
		std::string data;
		...
		Ref<SpatializationConfig> spatializationConfig = nullptr;

		//? this check may not be equally correct for Editor and Runtime, or if we change some stuff with asset handling
		if (Project::GetAssetManager()->IsAssetLoaded(metadata.Handle))
		{
			auto existingAsset = Project::GetAssetManager()->GetAsset(metadata.Handle);
			if (existingAsset && existingAsset->GetAssetType() == AssetType::SpatializationConfig)
				spatializationConfig = existingAsset.As<SpatializationConfig>();
		}
		else
		{
			spatializationConfig = Ref<SpatializationConfig>::Create();
		}

		if (!spatializationConfig) [[unlikely]]
			return false;

		if (DeserializeFromYAML(data, spatializationConfig))
		{
			asset = spatializationConfig;
			asset->Handle = metadata.Handle;
	...
	bool SpatializationConfigSerializer::SerializeToAssetPack(AssetHandle handle, FileStreamWriter& stream, AssetSerializationInfo& outInfo) const
	{
		if (Ref<SpatializationConfig> spatializationConfig = AssetManager::GetAsset<SpatializationConfig>(handle)) [[likely]]
		{
			outInfo.Offset = stream.GetStreamPosition();

			if (Serialization::Serialize(&stream, *spatializationConfig))
			{
				outInfo.Size = stream.GetStreamPosition() - outInfo.Offset;
	...
	Ref<Asset> SpatializationConfigSerializer::DeserializeFromAssetPack(FileStreamReader& stream, const AssetPackFile::AssetInfo& assetInfo) const
	{
		stream.SetStreamPosition(assetInfo.PackedOffset);
		
		auto spatializationConfig = Ref<SpatializationConfig>::Create();
		
		if (Serialization::Deserialize(&stream, *spatializationConfig))
			return spatializationConfig;

		return nullptr;
	}

	std::string SpatializationConfigSerializer::SerializeToYAML(Ref<SpatializationConfig> spatializationConfig) const
	{
		HZ_CORE_ASSERT(spatializationConfig);

		YAML::Emitter emitter;

		if (Serialization::Serialize(*spatializationConfig, emitter, Serialization::WrapInAMap))
			return std::string(emitter.c_str());

		return std::string();
	}

	bool SpatializationConfigSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SpatializationConfig> targetSpatializationConfig) const
	{
		HZ_CORE_ASSERT(targetSpatializationConfig);
		
		YAML::Node data = YAML::Load(yamlString);
		if (!data)
			return false;

		return Serialization::Deserialize(*targetSpatializationConfig, data);
	}
#endif
#if 0
	//////////////////////////////////////////////////////////////////////////////////
