...
#include "StarEngine/Audio/SoundBank.h"
#include "StarEngine/Audio/ResourceManager.h"

#include "StarEngine/Scene/Prefab.h"
#include "StarEngine/Scene/SceneSerializer.h"
#include "StarEngine/Scripting/ScriptAsset.h"
#include "StarEngine/Asset/MeshColliderAsset.h"

#include "StarEngine/Renderer/MaterialAsset.h"
...
#include "StarEngine/Utilities/YAMLSerializationHelpers.h"

#include "StarEngine/Serialization/TextureRuntimeSerializer.h"
...
namespace StarEngine {
	...
	std::string MaterialAssetSerializer::SerializeToYAML(Ref<MaterialAsset> materialAsset) const
	{
		...
		out << YAML::Key << "Material" << YAML::Value;
		{
			...
			Ref<Shader> transparentShader = Renderer::GetShaderLibrary()->Get("StarPBR_Transparent");
			bool transparent = materialAsset->GetMaterial()->GetShader() == transparentShader;
			SE_SERIALIZE_PROPERTY(Transparent, transparent, out);

			SE_SERIALIZE_PROPERTY(AlbedoColor, materialAsset->GetAlbedoColor(), out);
			SE_SERIALIZE_PROPERTY(Emission, materialAsset->GetEmission(), out);
			if (!transparent)
			{
				SE_SERIALIZE_PROPERTY(UseNormalMap, materialAsset->IsUsingNormalMap(), out);
				SE_SERIALIZE_PROPERTY(Metalness, materialAsset->GetMetalness(), out);
				SE_SERIALIZE_PROPERTY(Roughness, materialAsset->GetRoughness(), out);
			}
			else
			{
				SE_SERIALIZE_PROPERTY(Transparency, materialAsset->GetTransparency(), out);
			}

			{
				...
				SE_SERIALIZE_PROPERTY(AlbedoMap, albedoMapHandle, out);
			}
			if (!transparent)
			{
				{
					...
					SE_SERIALIZE_PROPERTY(NormalMap, normalMapHandle, out);
				}
				{
					...
					SE_SERIALIZE_PROPERTY(MetalnessMap, metalnessMapHandle, out);
				}
				{
					...
					SE_SERIALIZE_PROPERTY(RoughnessMap, roughnessMapHandle, out);
				}
			}

			SE_SERIALIZE_PROPERTY(MaterialFlags, materialAsset->GetMaterial()->GetFlags(), out);
	...
	void MaterialAssetSerializer::RegisterDependenciesFromYAML(const std::string& yamlString, AssetHandle handle) const
	{
		...
		SE_DESERIALIZE_PROPERTY(AlbedoMap, albedoMap, materialNode, (AssetHandle)0);
		SE_DESERIALIZE_PROPERTY(NormalMap, normalMap, materialNode, (AssetHandle)0);
		SE_DESERIALIZE_PROPERTY(MetalnessMap, metalnessMap, materialNode, (AssetHandle)0);
		SE_DESERIALIZE_PROPERTY(RoughnessMap, roughnessMap, materialNode, (AssetHandle)0);
	...
	bool MaterialAssetSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<MaterialAsset>& targetMaterialAsset, AssetHandle handle) const
	{
		...
		SE_DESERIALIZE_PROPERTY(Transparent, transparent, materialNode, false);

		targetMaterialAsset = Ref<MaterialAsset>::Create(transparent);
		targetMaterialAsset->Handle = handle;

		SE_DESERIALIZE_PROPERTY(AlbedoColor, targetMaterialAsset->GetAlbedoColor(), materialNode, glm::vec3(0.8f));
		SE_DESERIALIZE_PROPERTY(Emission, targetMaterialAsset->GetEmission(), materialNode, 0.0f);

		if (!transparent)
		{
			targetMaterialAsset->SetUseNormalMap(materialNode["UseNormalMap"] ? materialNode["UseNormalMap"].as<bool>() : false);
			SE_DESERIALIZE_PROPERTY(Metalness, targetMaterialAsset->GetMetalness(), materialNode, 0.0f);
			SE_DESERIALIZE_PROPERTY(Roughness, targetMaterialAsset->GetRoughness(), materialNode, 0.5f);
		}
		else
		{
			SE_DESERIALIZE_PROPERTY(Transparency, targetMaterialAsset->GetTransparency(), materialNode, 1.0f);
		}
		...
		SE_DESERIALIZE_PROPERTY(MaterialFlags, roughnessMap, materialNode, (AssetHandle)0);
	...
	std::string SoundConfigSerializer::SerializeToYAML(Ref<SoundConfig> soundConfig) const
	{
		...
		{
			SE_SERIALIZE_PROPERTY(AssetID, soundConfig->DataSourceAsset, out);
		}

		SE_SERIALIZE_PROPERTY(IsLooping, (bool)soundConfig->bLooping, out);
		SE_SERIALIZE_PROPERTY(VolumeMultiplier, soundConfig->VolumeMultiplier, out);
		SE_SERIALIZE_PROPERTY(PitchMultiplier, soundConfig->PitchMultiplier, out);
		SE_SERIALIZE_PROPERTY(MasterReverbSend, soundConfig->MasterReverbSend, out);
		SE_SERIALIZE_PROPERTY(LPFilterValue, soundConfig->LPFilterValue, out);
		SE_SERIALIZE_PROPERTY(HPFilterValue, soundConfig->HPFilterValue, out);
		...
		SE_SERIALIZE_PROPERTY(Enabled, soundConfig->bSpatializationEnabled, out);
		SE_SERIALIZE_PROPERTY(AttenuationModel, (int)spatialConfig->AttenuationMod, out);
		SE_SERIALIZE_PROPERTY(MinGain, spatialConfig->MinGain, out);
		SE_SERIALIZE_PROPERTY(MaxGain, spatialConfig->MaxGain, out);
		SE_SERIALIZE_PROPERTY(MinDistance, spatialConfig->MinDistance, out);
		SE_SERIALIZE_PROPERTY(MaxDistance, spatialConfig->MaxDistance, out);
		SE_SERIALIZE_PROPERTY(ConeInnerAngle, spatialConfig->ConeInnerAngleInRadians, out);
		SE_SERIALIZE_PROPERTY(ConeOuterAngle, spatialConfig->ConeOuterAngleInRadians, out);
		SE_SERIALIZE_PROPERTY(ConeOuterGain, spatialConfig->ConeOuterGain, out);
		SE_SERIALIZE_PROPERTY(DopplerFactor, spatialConfig->DopplerFactor, out);
		SE_SERIALIZE_PROPERTY(Rollor, spatialConfig->Rolloff, out);
		SE_SERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->bAirAbsorptionEnabled, out);
		SE_SERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->bSpreadFromSourceSize, out);
		SE_SERIALIZE_PROPERTY(SourceSize, spatialConfig->SourceSize, out);
		SE_SERIALIZE_PROPERTY(Spread, spatialConfig->Spread, out);
		SE_SERIALIZE_PROPERTY(Focus, spatialConfig->Focus, out);
	...
	bool SoundConfigSerializer::DeserializeFromYAML(const std::string& yamlString, Ref<SoundConfig> targetSoundConfig) const
	{
		...
		if (Application::IsRuntime())
		{
			...
			if (type == AssetType::Audio)
			{
				...
				SE_CORE_VERIFY(soundBank, "SoundBank is not loaded!");
		...
		else
		{
			if (Application::IsRuntime())
			{
				SE_CORE_ERROR("Tried to load invalid audio source asset (UUID {0}) in SoundConfig: {1}",
		...
		SE_DESERIALIZE_PROPERTY(IsLooping, targetSoundConfig->bLooping, data, false);
		SE_DESERIALIZE_PROPERTY(VolumeMultiplier, targetSoundConfig->VolumeMultiplier, data, 1.0f);
		SE_DESERIALIZE_PROPERTY(PitchMultiplier, targetSoundConfig->PitchMultiplier, data, 1.0f);
		SE_DESERIALIZE_PROPERTY(MasterReverbSend, targetSoundConfig->MasterReverbSend, data, 0.0f);
		SE_DESERIALIZE_PROPERTY(LPFilterValue, targetSoundConfig->LPFilterValue, data, 20000.0f);
		SE_DESERIALIZE_PROPERTY(HPFilterValue, targetSoundConfig->HPFilterValue, data, 0.0f);
		...
		if (spConfigData)
		{
			...
			SE_DESERIALIZE_PROPERTY(Enabled, targetSoundConfig->bSpatializationEnabled, spConfigData, false);
			...
			SE_DESERIALIZE_PROPERTY(MinGain, spatialConfig->MinGain, spConfigData, 0.0f);
			SE_DESERIALIZE_PROPERTY(MaxGain, spatialConfig->MaxGain, spConfigData, 1.0f);
			SE_DESERIALIZE_PROPERTY(MinDistance, spatialConfig->MinDistance, spConfigData, 1.0f);
			SE_DESERIALIZE_PROPERTY(MaxDistance, spatialConfig->MaxDistance, spConfigData, 1000.0f);
			SE_DESERIALIZE_PROPERTY(ConeInnerAngle, spatialConfig->ConeInnerAngleInRadians, spConfigData, 6.283185f);
			SE_DESERIALIZE_PROPERTY(ConeOuterAngle, spatialConfig->ConeOuterAngleInRadians, spConfigData, 6.283185f);
			SE_DESERIALIZE_PROPERTY(ConeOuterGain, spatialConfig->ConeOuterGain, spConfigData, 0.0f);
			SE_DESERIALIZE_PROPERTY(DopplerFactor, spatialConfig->DopplerFactor, spConfigData, 1.0f);
			SE_DESERIALIZE_PROPERTY(Rollor, spatialConfig->Rolloff, spConfigData, 1.0f);
			SE_DESERIALIZE_PROPERTY(AirAbsorptionEnabled, spatialConfig->bAirAbsorptionEnabled, spConfigData, true);
			SE_DESERIALIZE_PROPERTY(SpreadFromSourceSize, spatialConfig->bSpreadFromSourceSize, spConfigData, true);
			SE_DESERIALIZE_PROPERTY(SourceSize, spatialConfig->SourceSize, spConfigData, 1.0f);
			SE_DESERIALIZE_PROPERTY(Spread, spatialConfig->Spread, spConfigData, 1.0f);
			SE_DESERIALIZE_PROPERTY(Focus, spatialConfig->Focus, spConfigData, 1.0f);
		}
