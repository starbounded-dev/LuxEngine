...
namespace StarEngine {
	...
	{
		SE_CORE_INFO_TAG("AssetSystem", "Loading asset {}", alr.Handle);
		Ref<Asset> asset;
		if (asset = m_AssetPack->LoadAsset(alr.SceneHandle, alr.Handle); asset)
		{
			...
			SE_CORE_INFO_TAG("AssetSystem", "Finished loading asset {}", alr.Handle);
		}
		else
		{
			SE_CORE_ERROR_TAG("AssetSystem", "Failed to load asset {}", alr.Handle);
		}
		return asset;
	}
