...
namespace StarEngine {
	...
	{
		SE_PROFILE_THREAD("Asset Thread");

		while (m_Running)
		{
			SE_PROFILE_SCOPE("Asset Thread Queue");
	...
	{
		SE_PROFILE_FUNCTION("EditorAssetSystem::EnsureAllLoadedCurrent");
...
//	}
	...
	Ref<Asset> EditorAssetSystem::TryLoadData(AssetMetadata metadata)
	{
		...
		if (!metadata.IsValid())
		{
			SE_CORE_ERROR("Trying to load invalid asset");
			return asset;
		}

		SE_CORE_INFO_TAG("AssetSystem", "{}LOADING ASSET - {}", metadata.IsDataLoaded? "RE" : "", metadata.FilePath.string());
		...
		if (AssetImporter::TryLoadData(metadata, asset))
		{
			...
			SE_CORE_INFO_TAG("AssetSystem", "Finished loading asset {}", metadata.FilePath.string());
		}
		else
		{
			SE_CORE_ERROR_TAG("AssetSystem", "Failed to load asset {} ({})", metadata.Handle, metadata.FilePath);
		}
