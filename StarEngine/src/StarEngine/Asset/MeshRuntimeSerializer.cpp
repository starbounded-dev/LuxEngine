	...
	Ref<Asset> MeshRuntimeSerializer::DeserializeFromAssetPack(FileStreamReader& stream, const AssetPackFile::AssetInfo& assetInfo)
	{
		...
		if (hasAnimation || hasSkeleton)
		{
			...
			for (uint32_t i = 0; i < animationCount; i++)
			{
				...
				meshSource->m_Animations.emplace(hash, CreateScope<Animation>());
		...
		if (!meshSource->m_BoneInfluences.empty())
			meshSource->m_BoneInfluenceBuffer = VertexBuffer::Create(Buffer(meshSource->m_BoneInfluences.data(), (uint32_t)(meshSource->m_BoneInfluences.size() * sizeof(BoneInfluence))));
