...
#include "StarEngine/vendor/imgui-node-editor/widgets.h"
...
#pragma endregion
	...
	bool NodeGraphEditorBase::DrawPinPropertyEdit(PinPropertyContext& context)
	{
		...
		switch ((EPinType)pin->GetType())
		{
			...
			default:
				SE_CORE_ASSERT(false, "Unhandled pin type")
				break;
		}
	...
	{
		SE_CORE_ASSERT(node->Type != NodeType::Comment);
	...
	void NodeGraphEditorBase::DrawDeferredComboBoxes(PinPropertyContext& pinContext)
	{
		ed::Suspend();
		{
			...
			/// Enum combo box
			{
				...
				{
					bool changed = false;
					SE_CORE_ASSERT(pin->EnumTokens.has_value() && pin->EnumTokens.value() != nullptr);
	...
	void IONodeGraphEditor::DrawPropertyDetails(EPropertyType graphPropertyType, std::string_view propertyName)
	{
		...
		// Type & Value
		{
			...
			if (selected == -1)
			{
				SE_CORE_ERROR("Invalid type of Graph Input!");
				selected = 0;
			}
