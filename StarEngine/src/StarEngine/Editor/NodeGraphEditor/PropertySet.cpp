...
//}
	...
	choc::value::Value ParseCustomValueType(const choc::value::ValueView& value)
	{
		if (value.isVoid())
		{
			SE_CORE_WARN("Failed to deserialize custom value type, value is not an object.");
			return choc::value::Value(value);
		}
		if (value["TypeName"].isVoid())
		{
			SE_CORE_ASSERT(false, "Failed to deserialize custom value type, missing \"TypeName\" property.");
			return {};
		}
		...
		{
			SE_CORE_ASSERT(false, "Failed to load custom value type. It must be serialized as object.")
		}
	...
	void ParseCustomObject(PropertySet& ps, std::string_view name, const choc::value::ValueView& value)
	{
		if (value["Value"].isArray())
		{
			...
			SE_CORE_ASSERT(type == "AssetHandle", "The only custom type is supported for now is \"AssetHandle\".");
			SE_CORE_ASSERT(inArraySize > 0);
			...
			SE_CORE_ASSERT(valueArray.size() == inArraySize)
	...
	{
		SE_CORE_ASSERT(false, "Should use ..ExternalValue() serialization instead.");
	...
	std::string PropertySet::ToJSON() const { SE_CORE_ASSERT(false, "Should use ..ExternalValue() serialization instead."); return PropertySetToString(properties.get()); }
#endif
