		...
		public:
			...
			[[nodiscard]] static Node* SpawnNodeStatic(std::string_view category, std::string_view type)
			{
				...
				if (cat != Registry.end())
				{
					...
					{
						SE_CORE_ERROR("SpawnNodeStatic() - Category {0} does not contain node type {1}", category, type);
						return nullptr;
					}
				}

				SE_CORE_ERROR("SpawnNodeStatic() - Category {0} does not exist", category);
				return nullptr;
//				return T::SpawnNodeStatic();
			...
			static const std::vector<Token>* GetEnumTokens(std::string_view nodeName, std::string_view memberName)
			{
				...
				SE_CORE_ASSERT(nodeName.find("::") == std::string_view::npos);
				SE_CORE_ASSERT(memberName.find("::") == std::string_view::npos);
