...
namespace StarEngine {
	...
	void MeshColliderEditor::OnUpdate(Timestep ts)
	{
		...
		m_SelectedTabData->ViewportScene->OnRenderEditor(m_SelectedTabData->ViewportRenderer, m_SelectedTabData->ViewportCamera);
	}
	...
	{
		SE_PROFILE_FUNCTION("MeshColliderEditor::OnImGuiRender");
	...
	{
		SE_PROFILE_FUNCTION("MeshColliderEditor::RenderTab");
	...
	{
		SE_PROFILE_FUNCTION("MeshColliderEditor::RenderViewportPanel");
		...
		ImVec2 viewportSize = { maxBound.x - minBound.x, maxBound.y - minBound.y };

		if (viewportSize.x > 0 && viewportSize.y > 0)
		{
			tabData->ViewportScene->SetViewportBounds((uint32_t)minBound.x, (uint32_t)minBound.y, (uint32_t)maxBound.x, (uint32_t)maxBound.y);
			tabData->ViewportRenderer->SetViewportSize((uint32_t)viewportSize.x, (uint32_t)viewportSize.y);
			tabData->ViewportCamera.SetViewportBounds((uint32_t)minBound.x, (uint32_t)minBound.y, (uint32_t)maxBound.x, (uint32_t)maxBound.y);
	...
	{
		SE_PROFILE_FUNCTION("MeshColliderEditor::RenderSettingsPanel");
	...
	{
		SE_PROFILE_FUNCTION("MeshColliderEditor::UpdatePreviewEntity");
	...
	{
		SE_PROFILE_FUNCTION("MeshColliderEditor::RenderCookingOutput");
		...
		if (ImGui::BeginPopupModal("Mesh Collider Cooking Output", &m_IsCookingResultsOpen))
		{
			...
			auto getMessageForFailedCookingResult = [](ECookingResult cookingResult)
			{
				...
				{
					case StarEngine::ECookingResult::ZeroAreaTestFailed: return "Failed to find 4 initial vertices without a small triangle.";
					case StarEngine::ECookingResult::PolygonLimitReached:
					...
					case StarEngine::ECookingResult::LargeTriangle: return "Failed to cook triangle mesh because one or more triangles are too large. Tessellate the mesh to reduce triangle size.";
					case StarEngine::ECookingResult::InvalidMesh: return "Failed to cook mesh because an invalid mesh was provided. Please make sure you provide a valid mesh.";
					case StarEngine::ECookingResult::Failure: return "An unknown error occurred. Please check the output logs.";
				}
