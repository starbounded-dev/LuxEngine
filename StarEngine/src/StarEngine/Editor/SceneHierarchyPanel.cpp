...
//#include "StarEngine/Editor/NodeGraphEditor/AnimationGraph/AnimationGraphEditor.h"
#include "StarEngine/ImGui/CustomTreeNode.h"
#include "StarEngine/ImGui/PropertyGrid.h"
#include "StarEngine/ImGui/ImGuiWidgets.h"
...
namespace StarEngine {
	...
	{
		SE_PROFILE_FUNCTION("SceneHierarchyPanel::OnImGuiRender");
		if (m_IsWindow)
		{
	...
	void SceneHierarchyPanel::DrawEntityCreateMenu(Entity parent)
	{
		...
		if (ImGui::BeginMenu("3D"))
		{
			auto create3DEntity = [this](const char* entityName, const char* targetAssetName, const char* sourceAssetName)
			{
				...
				std::filesystem::path sourcePath = "Meshes/Source/Default";
				...
				else
				{
					if (std::filesystem::exists(Project::GetActiveAssetDirectory() / sourcePath / sourceAssetName))
					{
						...
						{
							Ref<StaticMesh> mesh = Project::GetEditorAssetManager()->CreateOrReplaceAsset<StaticMesh>(targetPath / targetAssetName, assetHandle, /*generateColliders=*/false);
							entity.AddComponent<StaticMeshComponent>(mesh->Handle);
						}
					}
					else
						SE_CONSOLE_LOG_WARN("Please import the default mesh source files to the following path: {0}", Project::GetActiveAssetDirectory() / sourcePath);
				}
				return entity;
			};
	...
	void SceneHierarchyPanel::DrawEntityNode(Entity entity, const std::string& searchFilter)
	{
		...
		ImGui::PushID((const void*)(uint64_t)entity.GetUUID());
		...
		const std::string strIDButtonBehavior = std::format("{0}BB", name);
		...
		bool previousState = ImGui::TreeNodeUpdateNextOpen(ImGui::GetID(strID.c_str()), ImGuiTreeNodeFlags_None);
				...
				if (isPrefab && isValidPrefab)
				{
					if (ImGui::MenuItem("Update Prefab"))
					{
						...
						if (prefab)
							...
							SE_ERROR("Prefab has invalid asset handle: {0}", prefabAssetHandle);
					}
				}
	...
	template<typename TComponent>
	void DrawMaterialTableFunc(SceneHierarchyPanel* _this, const std::vector<UUID>& entities, Ref<MaterialTable> meshMaterialTable, Ref<MaterialTable> localMaterialTable, const std::function<Ref<MaterialTable>(const TComponent&)>& componentMaterialTable)
	{
		if (UI::BeginTreeNode("Materials"))
		{
			UI::BeginPropertyGrid();
			...
			for (uint32_t i = 0; i < (uint32_t)localMaterialTable->GetMaterialCount(); i++)
			{
				...
				if (hasMeshMaterial && !hasLocalMaterial)
				{
					...
					UI::PropertyAssetReferenceTarget<MaterialAsset>(label.c_str(), meshMaterialName.c_str(), materialAssetHandle, [_this, &entities, i, localMaterialTable, &componentMaterialTable](Ref<MaterialAsset> materialAsset) mutable
					{
						Ref<Scene> context = _this->GetSceneContext();
						...
						for (auto entityID : entities)
						{
							Entity entity = context->GetEntityWithUUID(entityID);
							auto& component = entity.GetComponent<TComponent>();
							auto materialTable = componentMaterialTable(component);
				...
				else
				{
					...
					ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, entities.size() > 1 && _this->IsInconsistentPrimitive<AssetHandle, TComponent>([i, localMaterialTable, &componentMaterialTable](const TComponent& component)
					{
						Ref<MaterialTable> materialTable = componentMaterialTable(component);
					...
					UI::PropertyAssetReferenceTarget<MaterialAsset>(label.c_str(), nullptr, materialAssetHandle, [_this, &entities, i, localMaterialTable, &componentMaterialTable](Ref<MaterialAsset> materialAsset) mutable
					{
						Ref<Scene> context = _this->GetSceneContext();
	...
	void DrawSimpleAddComponentButton(SceneHierarchyPanel* _this, const std::string& name, Ref<Texture2D> icon = nullptr)
	{
		...
		ImGui::PushClipRect(rowAreaMin, rowAreaMax, false);
		bool isRowHovered, held;
		bool isRowClicked = ImGui::ButtonBehavior(ImRect(rowAreaMin, rowAreaMax), ImGui::GetID(name.c_str()), &isRowHovered, &held, ImGuiButtonFlags_AllowOverlap);
		ImGui::SetItemAllowOverlap();
		ImGui::PopClipRect();
	...
	void SceneHierarchyPanel::DrawComponents(const std::vector<UUID>& entityIDs)
	{
...

		{
			...
			if (ImGui::BeginPopup("AddComponentPanel", ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDocking))
			{
				...
				if (ImGui::BeginTable("##component_table", 2, ImGuiTableFlags_SizingStretchSame))
				{
					...
					DrawSimpleAddComponentButton<TileRendererComponent>(this, "Tile Renderer", EditorResources::StaticMeshIcon);
				...
				if (changed)
				{
					if (translationManuallyEdited || rotationManuallyEdited || scaleManuallyEdited)
					{
						...
						for (auto& entityID : entities)
						{
							...
							glm::quat oldRotation = component.GetRotation();
							glm::vec3 newRotationEuler = component.GetRotationEuler();
							if ((rotationAxes & UI::VectorAxis::X) != UI::VectorAxis::None)
								newRotationEuler.x = glm::radians(rotation.x);
							if ((rotationAxes & UI::VectorAxis::Y) != UI::VectorAxis::None)
								newRotationEuler.y = glm::radians(rotation.y);
							if ((rotationAxes & UI::VectorAxis::Z) != UI::VectorAxis::None)
								newRotationEuler.z = glm::radians(rotation.z);
							
							component.SetRotationEuler(newRotationEuler);
							Utils::ReorientAnimation(entity, oldRotation, component.GetRotation());
		...
		/*DrawComponent<AnimationComponent>("Animation", [&](AnimationComponent& firstComponent, const std::vector<UUID>& entities, const bool isMultiEdit)
		{
			...
			if (UI::PropertyAssetReference<AnimationGraphAsset>("Animation Graph", firstComponent.AnimationGraphHandle, "", &error, settings))
			{
				...
				for (auto entityID : entities)
				{
					...
					anim.RootBoneTransform = m_Context->FindRootBoneTransform(entity, anim.BoneEntityIds);
				}
			}
		...
		*/
		...
		DrawComponent<RigidBodyComponent>("RigidBody", [&](RigidBodyComponent& firstComponent, const std::vector<UUID>& entities, const bool isMultiEdit)
		{
			...
			if (!PhysicsLayerManager::IsLayerValid(firstComponent.LayerID))
			{
				for (auto& entityID : entityIDs)
				{
					...
					{
						auto actor = m_Context->GetPhysicsScene()->GetBody(entity);
			...
			if (firstComponent.BodyType != EBodyType::Static)
			{
				...
				if (UI::BeginTreeNode("Constraints", false))
				{
					...
					if (m_Context->IsPlaying())
					{
						auto physicsBody = m_Context->GetPhysicsScene()->GetBody(firstEntity);
		...
		DrawComponent<TileRendererComponent>("Tile Renderer", [&](TileRendererComponent& firstComponent, const std::vector<UUID>& entities, const bool isMultiEdit)
		{
			AssetHandle meshHandle = firstComponent.StaticMesh;
			auto mesh = AssetManager::GetAsset<StaticMesh>(firstComponent.StaticMesh);

			UI::BeginPropertyGrid();
			ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, isMultiEdit && IsInconsistentPrimitive<AssetHandle, TileRendererComponent>([](const TileRendererComponent& other) { return other.StaticMesh; }));
			UI::PropertyAssetReferenceError error;
			if (UI::PropertyAssetReferenceWithConversion<StaticMesh, MeshSource>("Static Mesh", meshHandle,
				[=](Ref<MeshSource> meshAsset)
			{
			...
			ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, isMultiEdit && IsInconsistentPrimitive<int, TileRendererComponent>([](const TileRendererComponent& other) { return other.Width; }));
			if (UI::Property("Width", firstComponent.Width))
			{
				for (auto& entityID : entities)
				{
					Entity entity = m_Context->GetEntityWithUUID(entityID);
					auto& c = entity.GetComponent<TileRendererComponent>();
					c.Width = firstComponent.Width;
					c.MaterialIDs.resize(firstComponent.Width* firstComponent.Height);
				}

				firstComponent.MaterialIDs.resize(firstComponent.Width * firstComponent.Height);
			}
			ImGui::PopItemFlag();

			ImGui::PushItemFlag(ImGuiItemFlags_MixedValue, isMultiEdit && IsInconsistentPrimitive<int, TileRendererComponent>([](const TileRendererComponent& other) { return other.Height; }));
			...
			{
				UI::BeginPropertyGrid();
				uint32_t materialCount = (uint32_t)firstComponent.Materials.size();
				if (UI::PropertyInput("Material Count", materialCount, 1, 1, ImGuiInputTextFlags_EnterReturnsTrue))
				{
					if (materialCount < 1)
						materialCount = 1;

					firstComponent.Materials.resize(materialCount);

					for (size_t i = 0; i < firstComponent.Materials.size(); i++)
					{
						if (!firstComponent.Materials[i])
							firstComponent.Materials[i] = Ref<MaterialTable>::Create();
					}
				...
				for (size_t i = 0; i < firstComponent.Materials.size(); i++)
				{
					...
					DrawMaterialTableFunc<TileRendererComponent>(this, entities, mesh->GetMaterials(), firstComponent.Materials[i],
						[i](const TileRendererComponent& component) { return component.Materials[i]; });
					UI::Separator();
