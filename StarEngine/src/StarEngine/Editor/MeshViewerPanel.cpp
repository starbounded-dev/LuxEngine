...
#include "StarEngine/Asset/MeshSerializer.h"
...
namespace StarEngine {
	...
	void MeshViewerPanel::OnUpdate(Timestep ts)
	{
		for (auto&& [name, sceneData] : m_OpenMeshes)
		{
			...
			if (sceneData->m_IsViewportVisible)
				sceneData->m_Scene->OnRenderEditor(sceneData->m_SceneRenderer, sceneData->m_Camera);
		}
	}
...

		{
...

			{
				...
				if (viewportSize.y > 0)
				{
					...
					sceneData->m_Camera.SetViewportBounds((uint32_t)minBound.x, (uint32_t)minBound.y, (uint32_t)maxBound.x, (uint32_t)maxBound.y);
	...
	void MeshViewerPanel::DrawMeshNode(const Ref<MeshSource>& meshSource, const Ref<Mesh>& mesh)
	{
#if 0
		// Mesh Hierarchy
		auto rootNode = meshSource->m_Scene->mRootNode;
		MeshNodeHierarchy(meshSource, mesh, 0);

		if (ImGui::Button("Create Mesh"))
		{
			std::filesystem::path meshSourcePath = meshSource->GetFilePath();
			std::filesystem::path directory = meshSourcePath.parent_path();
			std::string filename = std::format("{0}.hmesh", meshSourcePath.stem().string());
			Ref<Mesh> serializedMesh = Project::GetEditorAssetManager()->CreateOrReplaceAsset<Mesh>(directory / filename, mesh);
		}
#endif
	}
	...
	void MeshViewerPanel::MeshNodeHierarchy(const Ref<MeshSource>& meshAsset, Ref<Mesh> mesh, uint32_t nodeIndex, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		SE_CORE_ASSERT(meshAsset->m_NodeMap.find(node) != meshAsset->m_NodeMap.end());
