...
namespace StarEngine {
	...
	void PanelManager::RemovePanel(const char* strID)
	{
		...
		SE_CORE_ERROR_TAG("PanelManager", "Couldn't find panel with id '{0}'", strID);
	}
	...
	void PanelManager::Deserialize()
	{
		...
		if (!data["Panels"])
		{
			SE_CONSOLE_LOG_ERROR("Failed to load EditorLayout.yaml from {} because the file appears to be corrupted!", layoutPath.parent_path().string());
			return;
		}
