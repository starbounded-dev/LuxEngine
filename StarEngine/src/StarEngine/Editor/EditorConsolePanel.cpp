...
namespace StarEngine {
	...
	{
		SE_CORE_ASSERT(s_Instance == nullptr);
		s_Instance = this;
	...
	void EditorConsolePanel::RenderMenu(const ImVec2& size)
	{
		...
		const std::string clearOnPlayText = std::format("{} Clear on Play", m_ClearOnPlay ? SE_ICON_CHECK : SE_ICON_TIMES);
...

		{
			...
			if (ImGuiEx::ColoredButton(SE_ICON_INFO_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Info), textColor, buttonSize))
			...
			if (ImGuiEx::ColoredButton(SE_ICON_EXCLAMATION_TRIANGLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Warning), textColor, buttonSize))
			...
			if (ImGuiEx::ColoredButton(SE_ICON_EXCLAMATION_CIRCLE, GetToolbarButtonColor(m_MessageFilters & (int16_t)ConsoleMessageFlags::Error), textColor, buttonSize))
