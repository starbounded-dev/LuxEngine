...
static inline void OrShaderStage(nvrhi::ShaderType& dst, nvrhi::ShaderType src)
{
	using U = std::underlying_type_t<nvrhi::ShaderType>;
	dst = nvrhi::ShaderType(U(dst) | U(src));
}
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (uint32_t set = 0; set < m_ReflectionData.ShaderDescriptorSets.size(); set++)
		{
			...
			if (set == 0 && !m_ReflectionData.PushConstantRanges.empty())
			{
				...
				{
					OrShaderStage(bindingLayoutDesc.visibility, pushConstantRange.ShaderStage);
			...
			for (auto& [binding, uniformBuffer] : shaderDescriptorSet.UniformBuffers)
			{
				...
				OrShaderStage(bindingLayoutDesc.visibility, uniformBuffer.ShaderStage);
			...
			for (auto& [binding, storageBuffer] : shaderDescriptorSet.StorageBuffers)
			{
				...
				OrShaderStage(bindingLayoutDesc.visibility, storageBuffer.ShaderStage);
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				OrShaderStage(bindingLayoutDesc.visibility, imageSampler.ShaderStage);
