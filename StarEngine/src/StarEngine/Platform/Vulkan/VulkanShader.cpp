...
namespace StarEngine {
	...
	void VulkanShader::RT_Reload(const bool forceCompile)
	{
		...
		{
			SE_CORE_FATAL("Failed to recompile shader!");
		}
#endif
	}
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (uint32_t set = 0; set < m_ReflectionData.ShaderDescriptorSets.size(); set++)
		{
			...
			for (auto& [binding, storageBuffer] : shaderDescriptorSet.StorageBuffers)
			{
				...
				layoutBinding.pImmutableSamplers = nullptr;
				layoutBinding.binding = binding;
				SE_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				SE_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.SeparateTextures)
			{
				...
				SE_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.SeparateSamplers)
			{
				...
				SE_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.StorageBuffers.find(binding) == shaderDescriptorSet.StorageBuffers.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				SE_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
				SE_CORE_ASSERT(shaderDescriptorSet.SeparateSamplers.find(binding) == shaderDescriptorSet.SeparateSamplers.end(), "Binding is already present!");
			...
			SE_CORE_TRACE_TAG("Renderer", "Creating descriptor set {0} with {1} ubo's, {2} ssbo's, {3} samplers, {4} separate textures, {5} separate samplers and {6} storage images", set,
		...
		for (uint32_t set = 0; set < m_ReflectionData.ShaderDescriptorSets.size(); set++)
		{
			...
			if (set == 0 && !m_ReflectionData.PushConstantRanges.empty())
			{
				...
				{
					bindingLayoutDesc.visibility |= pushConstantRange.ShaderStage;
			...
			for (auto& [binding, uniformBuffer] : shaderDescriptorSet.UniformBuffers)
			{
				...
				bindingLayoutDesc.visibility |= uniformBuffer.ShaderStage;
			...
			for (auto& [binding, storageBuffer] : shaderDescriptorSet.StorageBuffers)
			{
				...
				bindingLayoutDesc.visibility |= storageBuffer.ShaderStage;
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				bindingLayoutDesc.visibility |= imageSampler.ShaderStage;
	...
	{
		SE_CORE_ASSERT(set < m_DescriptorSetLayouts.size());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		SE_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		SE_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
	...
	{
		SE_CORE_ASSERT(set < m_ReflectionData.ShaderDescriptorSets.size());
		SE_CORE_ASSERT(m_ReflectionData.ShaderDescriptorSets[set]);
		...
		{
			SE_CORE_WARN_TAG("Renderer", "Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
	...
	{
		SE_CORE_VERIFY(m_ShaderHandles.contains(type));
