...
namespace StarEngine::Utils {
	...
	void RetrieveDiagnosticCheckpoints()
	{
...

		{
			...
			vkGetQueueCheckpointDataNV(::StarEngine::VulkanContext::GetCurrentDevice()->GetGraphicsQueue(), &retrievedCount, data);
			SE_CORE_ERROR("RetrieveDiagnosticCheckpoints (Graphics Queue):");
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
				SE_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
			}
		}
		{
			const uint32_t checkpointCount = 4;
			...
			uint32_t retrievedCount = checkpointCount;
			vkGetQueueCheckpointDataNV(::StarEngine::VulkanContext::GetCurrentDevice()->GetComputeQueue(), &retrievedCount, data);
			SE_CORE_ERROR("RetrieveDiagnosticCheckpoints (Compute Queue):");
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
				SE_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
	...
	void VulkanCheckResult(VkResult result)
	{
		...
		{
			SE_CORE_ERROR("VkResult is '{0}'", Utils::VKResultToString(result));
	...
	void VulkanCheckResult(VkResult result, const char* file, int line)
	{
		if (result != VK_SUCCESS)
		{
			SE_CORE_ERROR("VkResult is '{0}' in {1}:{2}", Utils::VKResultToString(result), file, line);
			if (result == VK_ERROR_DEVICE_LOST)
			{
				using namespace std::chrono_literals;
				std::this_thread::sleep_for(3s);
