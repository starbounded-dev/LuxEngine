...
namespace StarEngine {
	...
	{
		s_Data = snew VulkanRendererData();
		...
		QuadVertex* data = snew QuadVertex[4];
	...
	void VulkanRenderer::RenderSubmeshInstanced(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<Mesh> mesh, Ref<MeshSource> meshSource, uint32_t submeshIndex, Ref<MaterialTable> materialTable, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, uint32_t boneTransformsOffset, uint32_t boneTransformsStride, uint32_t instanceCount)
	{
		...
		Renderer::Submit([renderCommandBuffer, pipeline, mesh, meshSource, submeshIndex, materialTable, transformBuffer, transformOffset, boneTransformsOffset, boneTransformsStride, instanceCount]() mutable
		{
			...
			if (submesh.IsRigged)
			{
				uint32_t boneTransformsOffsetStride[2] = { boneTransformsOffset, boneTransformsStride };
				vkCmdPushConstants(commandBuffer, layout, VK_SHADER_STAGE_VERTEX_BIT, pushConstantOffset, sizeof(uint32_t) * 2, boneTransformsOffsetStride);
	...
	void VulkanRenderer::RenderMeshWithMaterial(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<Mesh> mesh, Ref<MeshSource> meshSource, uint32_t submeshIndex, Ref<Material> material, Ref<VertexBuffer> transformBuffer, uint32_t transformOffset, uint32_t boneTransformsOffset, uint32_t boneTransformsStride, uint32_t instanceCount, Buffer additionalUniforms)
	{
		...
		{
			pushConstantBuffer.Allocate(additionalUniforms.Size + (isRigged ? sizeof(uint32_t) * 2 : 0));
			...
			{
				uint32_t boneTransformsOffsetStride[2] = { boneTransformsOffset, boneTransformsStride };
				pushConstantBuffer.Write(boneTransformsOffsetStride, sizeof(uint32_t) * 2, additionalUniforms.Size);
			}
		}
		...
		{
			SE_PROFILE_FUNC("VulkanRenderer::RenderMeshWithMaterial");
			SE_SCOPE_PERF("VulkanRenderer::RenderMeshWithMaterial");
	...
	{
		SE_CORE_ASSERT(staticMesh);
		SE_CORE_ASSERT(meshSource);
		SE_CORE_ASSERT(material);
	...
	void VulkanRenderer::RenderQuad(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<Material> material, const glm::mat4& transform)
	{
		...
		{
			SE_PROFILE_FUNC("VulkanRenderer::RenderQuad");
	...
	void VulkanRenderer::RenderGeometry(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<Material> material, Ref<VertexBuffer> vertexBuffer, Ref<IndexBuffer> indexBuffer, const glm::mat4& transform, uint32_t indexCount /*= 0*/)
	{
		...
		{
			SE_PROFILE_FUNC("VulkanRenderer::RenderGeometry");
	...
	void VulkanRenderer::SetUniformBuffer(Ref<UniformBuffer> uniformBuffer, uint32_t set)
	{
		Renderer::Submit([uniformBuffer, set]()
		{
			...
			SE_CORE_ASSERT(set == 1); // Currently we only bind to Renderer-maintaned UBs, which are in descriptor set 1
			...
			SE_CORE_WARN("VulkanRenderer - Updating descriptor set (VulkanRenderer::SetUniformBuffer)");
			VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
	...
	void VulkanRenderer::BlitImage(Ref<RenderCommandBuffer> commandBuffer, Ref<Image2D> sourceImage, Ref<Image2D> destinationImage)
	{
		...
		Renderer::Submit([commandBuffer, src = sourceImage.As<VulkanImage2D>(), dst = destinationImage.As<VulkanImage2D>()]
		{
			...
			if (!srcImage || !dstImage)
			{
				...
				SE_CORE_ERROR_TAG("Renderer", "[VulkanRenderer::BlitImage] Invalid images for blitting! srcImage={} dstImage={}", (uint64_t)srcImage, (uint64_t)dstImage);
				return;
			}
	...
	void VulkanRenderer::SubmitFullscreenQuad(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<Material> material)
	{
		...
		{
			SE_PROFILE_FUNC("VulkanRenderer::SubmitFullscreenQuad");
	...
	void VulkanRenderer::SetSceneEnvironment(Ref<SceneRenderer> sceneRenderer, Ref<Environment> environment, Ref<Image2D> shadow, Ref<Image2D> spotShadow)
	{
		...
		{
			SE_PROFILE_FUNC("VulkanRenderer::SetSceneEnvironment");

			const auto shader = Renderer::GetShaderLibrary()->Get("StarEnginePBR_Static");
	...
	void VulkanRenderer::BeginFrame()
	{
		Renderer::Submit([]()
		{
			SE_PROFILE_FUNC("VulkanRenderer::BeginFrame");
	...
	void VulkanRenderer::BeginRenderPass(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<RenderPass> renderPass, bool explicitClear)
	{
		...
		{
			SE_PROFILE_SCOPE_DYNAMIC(std::format("VulkanRenderer::BeginRenderPass ({})", renderPass->GetSpecification().DebugName).c_str());
			SE_CORE_TRACE_TAG("Renderer", "BeginRenderPass - {}", renderPass->GetSpecification().DebugName);
			...
			if (explicitClear)
			{
				...
				SE_CORE_ASSERT(clearValues.size() == totalAttachmentCount);
	...
	void VulkanRenderer::EndRenderPass(Ref<RenderCommandBuffer> renderCommandBuffer)
	{
		...
		{
			SE_PROFILE_FUNC("VulkanRenderer::EndRenderPass");
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> VulkanRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		SE_CORE_ASSERT(envEquirect->GetFormat() == ImageFormat::RGBA32F, "Texture is not HDR!");
	...
	Ref<TextureCube> VulkanRenderer::CreatePreethamSky(float turbidity, float azimuth, float inclination)
	{
		...
		Renderer::Submit([preethamSkyComputePipeline, environmentMap, cubemapSize, params]() mutable
		{
			...
			nvrhi::BindingLayoutHandle bindingLayout = shader->GetDescriptorSetLayout();
			nvrhi::BindingSetDesc bindingSetDesc;
			bindingSetDesc.bindings = {
				nvrhi::BindingSetItem::Texture_SRV(0, environmentMap->GetHandle())
			};
