...
namespace StarEngine {
	...
	void DescriptorSetManager::SetInput(std::string_view name, Ref<UniformBufferSet> uniformBufferSet)
	{
		...
		{
			SE_CORE_WARN_TAG("Renderer", "[RenderPass ({})] Input {} not found", m_Specification.DebugName, name);
		}
	}
	...
	bool DescriptorSetManager::Validate()
	{
		...
		for (uint32_t set = m_Specification.StartSet; set <= m_Specification.EndSet; set++)
		{
			...
			{
				SE_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] No input resources for Set {}", m_Specification.DebugName, set);
				return false;
			}
			...
			for (auto&& [name, inputDecl] : shaderDescriptor.InputDeclarations)
			{
				...
				{
					SE_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] No input resource for {}.{}", m_Specification.DebugName, set, binding);
					SE_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] Required resource is {} ({})", m_Specification.DebugName, name, (int)inputDecl.Type);
					return false;
				}
				...
				{
					SE_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] Required resource is wrong type! {} but needs {}", m_Specification.DebugName, (uint16_t)resource.Type, (int)inputDecl.Type);
					return false;
				}

				if (resource.Type != RenderResourceType::Image2D && resource.Input[0] == nullptr)
				{
					SE_CORE_ERROR_TAG("Renderer", "[RenderPass ({})] Resource is null! {} ({}.{})", m_Specification.DebugName, name, set, binding);
					return false;
				}
			}
		}
				...
				for (const auto& [binding, input] : setData)
				{
					...
					switch (input.Type)
					{
						...
						case RenderResourceType::TextureCube:
						{
							...
							SE_CORE_ASSERT(bindingSetItem.dimension == nvrhi::TextureDimension::TextureCube);
			...
			for (uint32_t frameIndex = 0; frameIndex < descriptorSetCount; frameIndex++)
			{
				...
				for (const auto& [binding, input] : setData)
				{
					...
					switch (input.Type)
					{
						...
						case RenderPassResourceType::ImageView:
						{
	...
	void DescriptorSetManager::InvalidateAndUpdate()
	{
		...
		{
			SE_CORE_INFO_TAG("Renderer", "DescriptorSetManager::InvalidateAndUpdate ({}) - updating {} descriptors (frameIndex={})", m_Specification.DebugName, InvalidatedInputResources.size(), currentFrameIndex);
			Bake();
		}
					...
					switch (input.Type)
					{
						...
						case RenderPassResourceType::ImageView:
						{
							if (input.Input.size() > 1)
							{
								...
								for (size_t i = 0; i < input.Input.size(); i++)
								{
									Ref<VulkanImageView> imageView = input.Input[i].As<VulkanImageView>();
									imageInfoStorage[imageInfoStorageIndex][i] = imageView->GetDescriptorInfoVulkan();
									wd.ResourceHandles[i] = imageInfoStorage[imageInfoStorageIndex][i].imageView;
								}
								writeDescriptor.pImageInfo = imageInfoStorage[imageInfoStorageIndex].data();
								imageInfoStorageIndex++;
							...
							{
								Ref<VulkanImageView> imageView = input.Input[0].As<VulkanImageView>();
								writeDescriptor.pImageInfo = &imageView->GetDescriptorInfoVulkan();
								wd.ResourceHandles[0] = writeDescriptor.pImageInfo->imageView;
							}
