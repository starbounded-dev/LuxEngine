...
#define SE_HAS_AFTERMATH !HZ_DIST
...
namespace StarEngine {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			SE_CORE_INFO_TAG("Renderer", "Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		SE_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		SE_CORE_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				SE_CORE_INFO_TAG("Renderer", "Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					SE_CORE_INFO_TAG("Renderer", "  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		SE_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		SE_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
		...
		if (canEnableAftermath)
		{
			...
			GpuCrashTracker* gpuCrashTracker = snew GpuCrashTracker();
		...
		SE_CORE_ASSERT(result == VK_SUCCESS);
	...
	Ref<VulkanCommandPool> VulkanDevice::GetThreadLocalCommandPool()
	{
		...
		SE_CORE_VERIFY(m_CommandPools.find(threadID) != m_CommandPools.end());
	...
	{
		auto device = VulkanContext::GetCurrentDevice();
		SE_CORE_VERIFY(queue == device->GetGraphicsQueue());
		auto vulkanDevice = device->GetVulkanDevice();

		const uint64_t DEFAULT_FENCE_TIMEOUT = 100000000000;

		SE_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
