...
namespace std {
	template<>
	struct hash<StarEngine::IncludeData>
	{
		size_t operator()(const StarEngine::IncludeData& data) const noexcept
		{
...
namespace StarEngine {
		...
		while (startOfShaderStage != std::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "pragma")
				{
					...
					{
						SE_CORE_VERIFY(tokens[++index] == ":", "Stage pragma is invalid");

						// Skipped ':'
						const std::string_view stage(tokens[++index]);
						SE_CORE_VERIFY(stage == "vert" || stage == "frag" || stage == "comp", "Invalid shader type specified");
	...
	std::map<nvrhi::ShaderType, std::string> ShaderPreprocessor::PreprocessShader(const std::string& source, std::unordered_set<std::string>& specialMacros)
	{
		...
		SE_CORE_ASSERT(newSource.size(), "Shader is empty!");
		...
		if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
		{
			...
			SE_CORE_VERIFY(tokens.size() >= 3 && tokens[1] == "version", "Invalid #version encountered or #version is NOT encounted first.");
		...
		SE_CORE_VERIFY(stagePositions.size(), "Could not pre-process shader! There are no known stages defined in file.");
