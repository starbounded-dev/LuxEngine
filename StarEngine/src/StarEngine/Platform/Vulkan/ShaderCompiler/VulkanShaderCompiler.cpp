...
namespace StarEngine {
	...
	namespace Utils {
		...
		static ShaderUniformType SPIRTypeToShaderUniformType(spirv_cross::SPIRType type)
		{
			...
			SE_CORE_ASSERT(false, "Unknown type!");
	...
	bool VulkanShaderCompiler::Reload(bool forceCompile)
	{
		...
		SE_CORE_VERIFY(source.size(), "Failed to load shader!");

		SE_CORE_TRACE_TAG("Renderer", "Compiling shader: {}", m_ShaderSourcePath.string());
	...
	std::map<nvrhi::ShaderType, std::string> VulkanShaderCompiler::PreProcessGLSL(const std::string& source)
	{
		...
		for (auto& [stage, shaderSource] : shaderSources)
		{
			...
			options.AddMacroDefinition(std::string(ShaderUtils::ShaderStageToShaderMacro(stage)));
	...
	std::map<nvrhi::ShaderType, std::string> VulkanShaderCompiler::PreProcessHLSL(const std::string& source)
	{
		...
		for (auto& [stage, shaderSource] : shaderSources)
		{
			...
			{
				SE_CORE_ERROR_TAG("Renderer", error);
			}
	...
	std::string VulkanShaderCompiler::Compile(std::vector<uint32_t>& outputBinary, const nvrhi::ShaderType stage, CompilationOptions options) const
	{
		...
		else if (m_Language == ShaderUtils::SourceLang::HLSL)
		{
			...
			char tempfileName[] = "star-hlsl-XXXXXX.spv";
	...
	bool VulkanShaderCompiler::CompileOrGetVulkanBinary(nvrhi::ShaderType stage, std::vector<uint32_t>& outputBinary, bool debug, nvrhi::ShaderType changedStages, bool forceCompile)
	{
		...
		if (outputBinary.empty())
		{
			...
			if (std::string error = Compile(outputBinary, stage, options); error.size())
			{
				...
				if (outputBinary.empty())
				{
					SE_CONSOLE_LOG_ERROR("Failed to compile shader and couldn't find a cached version.");
				}
				else
				{
					SE_CONSOLE_LOG_ERROR("Failed to compile {}:{} so a cached version was loaded instead.", m_ShaderSourcePath.string(), nvrhi::utils::ShaderStageToString(stage));
#if 0
			...
			else // Compile success
			{
				...
				if (!f)
					SE_CORE_ERROR("Failed to cache shader binary!");
	...
	bool VulkanShaderCompiler::TryReadCachedReflectionData()
	{
		...
		bool validHeader = memcmp(&header, "SESR", 4) == 0;
		SE_CORE_VERIFY(validHeader);
		if (!validHeader)
			return false;
	...
	void VulkanShaderCompiler::Reflect(nvrhi::ShaderType shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		SE_CORE_INFO_TAG("Renderer", " Vulkan Shader Reflection");
		SE_CORE_INFO_TAG("Renderer", "===========================");
		SE_CORE_INFO_TAG("Renderer", m_ShaderSourcePath.string());

		spirv_cross::Compiler compiler(shaderData);
		auto resources = compiler.get_shader_resources();

		SE_CORE_INFO_TAG("Renderer", "Uniform Buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			if (activeBuffers.size())
			{
				...
				SE_CORE_INFO_TAG("Renderer", "  {0} ({1}, {2})", name, descriptorSet, binding);
				SE_CORE_INFO_TAG("Renderer", "  Member Count: {0}", memberCount);
				SE_CORE_INFO_TAG("Renderer", "  Size: {0}", size);
				SE_CORE_INFO_TAG("Renderer", "-------------------");
			}
		}

		SE_CORE_INFO_TAG("Renderer", "Storage Buffers:");
		for (const auto& resource : resources.storage_buffers)
		{
			...
			if (activeBuffers.size())
			{
				...
				SE_CORE_INFO_TAG("Renderer", "  Member Count: {0}", memberCount);
				SE_CORE_INFO_TAG("Renderer", "  Size: {0}", size);
				SE_CORE_INFO_TAG("Renderer", "  ReadOnly: {0}", readOnly);
				SE_CORE_INFO_TAG("Renderer", "-------------------");
			}
		}

		SE_CORE_INFO_TAG("Renderer", "Push Constant Buffers:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			SE_CORE_INFO_TAG("Renderer", "  Name: {0}", bufferName);
			SE_CORE_INFO_TAG("Renderer", "  Member Count: {0}", memberCount);
			SE_CORE_INFO_TAG("Renderer", "  Size: {0}", bufferSize);
		...
		SE_CORE_INFO_TAG("Renderer", "Sampled Images:");
		...
		SE_CORE_INFO_TAG("Renderer", "Separate Images:");
		...
		SE_CORE_INFO_TAG("Renderer", "Separate Samplers:");
		...
		SE_CORE_INFO_TAG("Renderer", "Storage Images:");
		...
		SE_CORE_INFO_TAG("Renderer", "Special macros:");
		for (const auto& macro : m_AcknowledgedMacros)
		{
			SE_CORE_INFO_TAG("Renderer", "  {0}", macro);
		}
