...
namespace StarEngine {
		...
		for (auto& [filepath, shader] : shaderCache)
		{
			...
			for (auto& [stage, stageData] : shader)
			{
				...
				for (auto& header : stageData.Headers)
				{
					...
					SE_SERIALIZE_PROPERTY(HeaderPath, header.IncludedFilePath.string(), out);
					SE_SERIALIZE_PROPERTY(IncludeDepth, header.IncludeDepth, out);
					SE_SERIALIZE_PROPERTY(IsRelative, header.IsRelative, out);
					SE_SERIALIZE_PROPERTY(IsGaurded, header.IsGuarded, out);
					SE_SERIALIZE_PROPERTY(HashValue, header.HashValue, out);
	...
	void VulkanShaderCache::Deserialize(std::map<std::string, std::map<nvrhi::ShaderType, StageData>>& shaderCache)
	{
		...
		if (handles.IsNull())
		{
			SE_CORE_ERROR("[ShaderCache] Shader Registry is invalid.");
			return;
		}
		...
		if (handles.IsMap())
		{
			SE_CORE_ERROR("[ShaderCache] Old Shader Registry format.");
			return;
		}
		...
		{
			std::string path;
			SE_DESERIALIZE_PROPERTY(ShaderPath, path, shader, std::string());
			for (auto stage : shader["Stages"]) //Stages
			{
				std::string stageType;
				uint32_t stageHash;
				SE_DESERIALIZE_PROPERTY(Stage, stageType, stage, std::string());
				SE_DESERIALIZE_PROPERTY(StageHash, stageHash, stage, 0u);
				...
				for (auto header : stage["Headers"])
				{
					...
					bool isGuarded;
					uint32_t hashValue;
					SE_DESERIALIZE_PROPERTY(HeaderPath, headerPath, header, std::string());
					SE_DESERIALIZE_PROPERTY(IncludeDepth, includeDepth, header, 0u);
					SE_DESERIALIZE_PROPERTY(IsRelative, isRelative, header, false);
					SE_DESERIALIZE_PROPERTY(IsGaurded, isGuarded, header, false);
					SE_DESERIALIZE_PROPERTY(HashValue, hashValue, header, 0u);
