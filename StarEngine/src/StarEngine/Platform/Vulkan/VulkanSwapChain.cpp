...
namespace StarEngine {
	...
	bool VulkanSwapChain::Create(uint32_t width, uint32_t height)
	{
		...
		const auto& deviceParams = Application::GetGraphicsDeviceManager()->GetDeviceParams();
		auto device = Application::GetGraphicsDevice();

		VulkanDeviceManager* vulkanDeviceManager = (VulkanDeviceManager*)Application::GetGraphicsDeviceManager();
		...
		{
			SE_CORE_ERROR("Failed to create a Vulkan swap chain, error code = {}", nvrhi::vulkan::resultToString(VkResult(res)));
			return false;
		}
	...
	{
		SE_PROFILE_FUNCTION("VulkanSwapChain::BeginFrame");
	...
	{
		SE_PROFILE_FUNCTION("VulkanSwapChain::Present");
		...
		{
			SE_PROFILE_FUNCTION("PresentKHR");
			...
			SE_CORE_VERIFY(res == vk::Result::eSuccess || res == vk::Result::eErrorOutOfDateKHR);
		...
		{
			SE_PROFILE_SCOPE("WaitEventQuery");
		...
		{
			SE_PROFILE_SCOPE("OtherEventQuery");
