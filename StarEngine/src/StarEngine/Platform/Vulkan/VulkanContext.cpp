...
namespace StarEngine {
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDebugReportCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
	{
		...
		SE_CORE_WARN_TAG("Renderer", "VulkanDebugCallback:\n  Object Type: {0}\n  Message: {1}", objectType, pMessage);
	...
	static bool CheckDriverAPIVersionSupport(uint32_t minimumSupportedVersion)
	{
		...
		{
			SE_CORE_FATAL("Incompatible Vulkan driver version!");
			SE_CORE_FATAL("  You have {}.{}.{}", VK_API_VERSION_MAJOR(instanceVersion), VK_API_VERSION_MINOR(instanceVersion), VK_API_VERSION_PATCH(instanceVersion));
			SE_CORE_FATAL("  You need at least {}.{}.{}", VK_API_VERSION_MAJOR(minimumSupportedVersion), VK_API_VERSION_MINOR(minimumSupportedVersion), VK_API_VERSION_PATCH(minimumSupportedVersion));
	...
	{
		SE_CORE_INFO_TAG("Renderer", "VulkanContext::Create");

		SE_CORE_ASSERT(glfwVulkanSupported(), "GLFW must support Vulkan!");

		if (!CheckDriverAPIVersionSupport(VK_API_VERSION_1_2))
		{
#ifdef SE_PLATFORM_WINDOWS
			MessageBox(nullptr, L"Incompatible Vulkan driver version.\nUpdate your GPU drivers!", L"StarEngine Error", MB_OK | MB_ICONERROR);
#else
			SE_CORE_ERROR("Incompatible Vulkan driver version.\nUpdate your GPU drivers!");
#endif
		...
		appInfo.pApplicationName = "StarEngine";
		appInfo.pEngineName = "StarEngine";
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_xcb_surface"
		...
		if (s_Validation)
		{
			...
			SE_CORE_INFO_TAG("Renderer", "Vulkan Instance Layers:");
			for (const VkLayerProperties& layer : instanceLayerProperties)
			{
				SE_CORE_INFO_TAG("Renderer", "  {0}", layer.layerName);
			...
			{
				SE_CORE_ERROR_TAG("Renderer", "Validation layer VK_LAYER_KHRONOS_validation not present, validation is disabled");
			}
		}
		...
		if (s_Validation)
		{
			...
			SE_CORE_ASSERT(vkCreateDebugReportCallbackEXT != NULL, "");
			...
			SE_CORE_ASSERT(vkCreateDebugUtilsMessengerEXT != NULL, "");
