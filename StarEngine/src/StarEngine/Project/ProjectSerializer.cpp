...
namespace StarEngine {
	...
	void ProjectSerializer::Serialize(const std::filesystem::path& filepath)
	{
		...
		out << YAML::Key << "Project" << YAML::Value;
		{
			...
			{
				out << YAML::BeginMap;
				auto userConfig = AudioEngine::GetEngine();
				SE_SERIALIZE_PROPERTY(FileStreamingDurationThreshold, userConfig.FileStreamingDurationThreshold, out);
				out << YAML::EndMap;
			}
	...
	bool ProjectSerializer::SerializeRuntime(const std::filesystem::path& filepath)
	{
		...
		// Start Scene
		{
			...
			if (startSceneID == 0)
			{
				SE_CORE_ERROR("Error building runtime project - no start scene could be found! (StartScene: {})", config.StartScene);
				return false;
			}
	...
	bool ProjectSerializer::Deserialize(const std::filesystem::path& filepath)
	{
		...
		{
			auto userConfig = MiniAudioEngine::Get().GetUserConfiguration();
			SE_DESERIALIZE_PROPERTY(FileStreamingDurationThreshold, userConfig.FileStreamingDurationThreshold, audioNode, userConfig.FileStreamingDurationThreshold);
	...
	bool ProjectSerializer::DeserializeRuntime(const std::filesystem::path& filepath)
	{
		...
		SE_CORE_TRACE_TAG("Project", "Deserializing Project from {}", filepath.string());
		...
		if (!validHeader)
		{
			...
			SE_CORE_ERROR_TAG("Project", "Project file has invalid header ({})", invalidHeader);
			return false;
		}
		...
		{
			SE_CORE_ERROR_TAG("Project", "Project version {} is not compatible with current version {}", projectInfo.Header.Version, current.Header.Version);
			return false;
		}
