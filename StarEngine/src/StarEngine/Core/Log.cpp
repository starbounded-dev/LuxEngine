...
/*
#ifndef SE_HEADLESS
#include "StarEngine/Editor/EditorConsole/EditorConsoleSink.h"
#endif
*/
#include <filesystem>

#define SE_HAS_CONSOLE !SE_DIST
...
namespace StarEngine {
	...
	void Log::Init()
	{
		...
		std::vector<spdlog::sink_ptr> starSinks =
		{
		...
		starSinks[0]->set_pattern("[%T] [%l] %n: %v");
		appSinks[0]->set_pattern("[%T] [%l] %n: %v");
		/*
#if SE_HAS_CONSOLE
		starSinks[1]->set_pattern("%^[%T] %n: %v%$");
		appSinks[1]->set_pattern("%^[%T] %n: %v%$");
		for (auto sink : editorConsoleSinks)
			sink->set_pattern("%^%v%$");
#endif*/

		s_CoreLogger = std::make_shared<spdlog::logger>("STARENGINE", starSinks.begin(), starSinks.end());
